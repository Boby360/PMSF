...
var gruntCharacterTypes = [
    "unset",
    "Team Leader(s)",
    "Team GO Rocket Grunt(s)",
    "Arlo",
    "Cliff",
    "Sierra",
    "Giovanni",
...
function previewPoiSurrounding(event) {
    // eslint-disable-line no-unused-vars
    var form = $(event.target).parent().parent();
    var input = event.target;
    var reader = new FileReader();
...
function excludePokemon(id) {
    // eslint-disable-line no-unused-vars
    $selectExclude
        .val($selectExclude.val().split(",").concat(id).join(","))
        .trigger("change");
    $(
        '#exclude-pokemon .pokemon-list .pokemon-icon-sprite[data-value="' +
            id +
            '"]'
    ).addClass("active");
    clearStaleMarkers();
...
function notifyAboutPokemon(id) {
    // eslint-disable-line no-unused-vars
    $selectPokemonNotify
        .val($selectPokemonNotify.val().split(",").concat(id).join(","))
        .trigger("change");
    $(
        '#notify-pokemon .pokemon-list .pokemon-icon-sprite[data-value="' +
            id +
            '"]'
    ).addClass("active");
}
...
function removePokestopMarker(pokestopId) {
    // eslint-disable-line no-unused-vars
    if (mapData.pokestops[pokestopId].marker.placementRangeCircle) {
        markers.removeLayer(
            mapData.pokestops[pokestopId].marker.placementRangeCircle
        );
...
function removeGymMarker(gymId) {
    // eslint-disable-line no-unused-vars
...
function initMap() {
        ...
        if ($(".submit-on-off-button").hasClass("on")) {
            ...
            $("#submitModal").on("shown.bs.modal", function (event) {
                $("#submitModal .pokemon-list-cont").each(function (index) {
                    ...
                    var monList = new List(
                        "pokemon-list-cont-6" + index,
                        options
                    ); // eslint-disable-line no-unused-vars
                });
            });
    ...
    $.getJSON("static/dist/data/searchmarkerstyle.min.json").done(function (
        data
    ) {
        ...
        $selectLocationIconMarker
            .val(Store.get("locationMarkerStyle"))
            .trigger("change");
    });
...
var mapboxPogoDynamicConfig =
    currentHour >= 6 && currentHour < 19
        ? getTileLayerConfig("mapboxPogo")
        : getTileLayerConfig("mapboxPogoDark"); // eslint-disable-line no-unused-vars
    ...
    switch (selectedStyle) {
        ...
        default:
            if (selectedStyle.includes("mapbox")) {
                tileLayerConfig = L.tileLayer(
                    mapStyleList[selectedStyle]["url"] +
                        mapStyleList[selectedStyle]["key"],
                    {
                        attribution: mapStyleList[selectedStyle]["attribution"],
                        maxZoom: maxZoom,
                        maxNativeZoom:
                            mapStyleList[selectedStyle]["maxnativezoom"],
                    }
                );
                break;
            }
...
function setTileLayer(layername) {
    if (
        map.hasLayer(getTileLayerConfig(_oldlayer)) &&
        getTileLayerConfig(_oldlayer) !== getTileLayerConfig(layername)
    ) {
...
function createLocationMarker() {
    ...
    var locationMarker = L.marker([lat, lng])
        .addTo(markersnotify)
        .bindPopup("<div><b>My location</b></div>");
...
function cellLabel(
    ...
    if (totalCount >= 20 && gymCount >= 3) {
        html +=
            "<div><center><b>" +
            i8ln("Max amount of Gyms reached") +
            "</b></center></div>";
        ...
        html +=
            ...
            count +
            " " +
            i8ln("more Pokéstop(s) until new gym") +
            "</b></center></div>";
    ...
    if (sponsoredStopCount > 0) {
        html +=
            "<div>" +
            i8ln("Sponsored Pokéstops in cell") +
            ": <b>" +
            sponsoredStopCount +
            "</b></div>";
    }
    ...
    if (sponsoredStopCount > 0 || sponsoredGymCount > 0) {
        html +=
            "<div>" +
            i8ln("Total (excluding sponsored)") +
            ": <b>" +
            totalCount +
            "</b></div>";
    } else {
...
function showS2Cells(level, style) {
    ...
    function addPoly(cell) {
        ...
        if (
            cell.level === 14 &&
            Store.get("showPokestops") &&
            Store.get("showGyms")
        ) {
            if (
                (totalCount === 1 && gymCount === 0) ||
                (totalCount === 5 && gymCount === 1) ||
                (totalCount === 19 && gymCount === 2)
            ) {
                filledStyle = { fillColor: s2Colors[1], fillOpacity: 0.3 };
            } else if (
                (totalCount === 4 && gymCount === 1) ||
                (totalCount === 18 && gymCount === 2)
            ) {
        ...
        if (
            ...
            Store.get("showPokestops") &&
            Store.get("showGyms") &&
            !$(".submit-on-off-button").hasClass("on")
        ) {
            poly.bindPopup(
                cellLabel(
                    stopCount,
...
function buildNestPolygons() {
    if (
        !Store.get(["showNestPolygon"]) ||
        !Store.get(["showNests"]) ||
        nestGeoJSONfile.trim() === ""
    ) {
        return false;
    }
        ...
        var hearts = {
            ...
            initialize: function () {
                ...
                for (var a = 0; a < this.maxHearts; a++) {
                    var scale =
                        Math.random() * (1 - this.minScale) + this.minScale;
                    this.hearts.push({
...
function initSidebar() {
    ...
    $("#placement-ranges-switch").prop(
        "checked",
        Store.get("showPlacementRanges")
    );
    ...
    $("#missing-iv-only-switch").prop(
        "checked",
        Store.get("showMissingIVOnly")
    );
    ...
    $("#no-independant-pvp-switch").prop(
        "checked",
        Store.get("showIndependantPvpAndStats")
    );
    ...
    $("#start-at-user-location-switch").prop(
        "checked",
        Store.get("startAtUserLocation")
    );
    $("#start-at-last-location-switch").prop(
        "checked",
        Store.get("startAtLastLocation")
    );
    $("#follow-my-location-switch").prop(
        "checked",
        Store.get("followMyLocation")
    );
    ...
    $("#eventstops-timer-switch").prop(
        "checked",
        Store.get("showEventStopsTimer")
    );
    ...
    $("#notification-switch").prop(
        "checked",
        Store.get("remember_notification_notify")
    );
    ...
    if (document.getElementById("next-location")) {
        ...
        searchform.addEventListener("input", async (event) => {
            ...
            $.each(results, function (key, val) {
                $("#search-places-results").append(
                    '<li class="place-result" data-lat="' +
                        val.y +
                        '" data-lon="' +
                        val.x +
                        '"><span class="place-result" onclick="centerMapOnCoords(event);">' +
                        val.label +
                        "</span></li>"
                );
            });
...
function pokemonLabel(item) {
    ...
    var rarityDisplay = item["pokemon_rarity"]
        ? "(" + item["pokemon_rarity"] + ")"
        : "";
    ...
    var pMove1 =
        moves[item["move_1"]] !== undefined
            ? i8ln(moves[item["move_1"]]["name"])
            : "unknown";
    var pMove2 =
        moves[item["move_2"]] !== undefined
            ? i8ln(moves[item["move_2"]]["name"])
            : "unknown";
    var pMoveType1 = "";
    var pMoveType2 = "";
    var weight =
    ...
    $.each(types, function (index, type) {
        typesDisplay +=
            ...
            getIcon(
                ...
                "type",
                ".png",
                getKeyByValue(pokemonTypes, type.type)
            ) +
            '" style="height:20px;">';
    });
    ...
    if (atk != null && def != null && sta != null) {
        ...
        if (pMove1 !== "unknown") {
            pMoveType1 =
                '<img style="position:relative;top:3px;left:2px;height:15px;" src="' +
                getIcon(
                    iconpath.type,
                    "type",
                    ".png",
                    getKeyByValue(pokemonTypes, moves[item["move_1"]]["type"])
                ) +
                '">';
        }
        if (pMove2 !== "unknown") {
            pMoveType2 =
                ...
                getIcon(
                    ...
                    "type",
                    ".png",
                    getKeyByValue(pokemonTypes, moves[item["move_2"]]["type"])
                ) +
                '">';
        }
        ...
        if (
            !noCatchRates &&
            item["catch_rate_1"] != null &&
            item["catch_rate_2"] != null &&
            item["catch_rate_3"] != null
        ) {
            catchRates =
                "<div>" +
                '<img src="static/images/pokeball-1.png" style="height:14px;position:relative;top:2px;"> ' +
                (item["catch_rate_1"] * 100).toFixed(1) +
                "% " +
                '<img src="static/images/greatball.png" style="height:14px;position:relative;top:2px;"> ' +
                (item["catch_rate_2"] * 100).toFixed(1) +
                "% " +
                '<img src="static/images/ultraball.png" style="height:14px;position:relative;top:2px;"> ' +
                (item["catch_rate_3"] * 100).toFixed(1) +
                "%" +
                "</div>";
        }
        ...
        if (item["size"] != null) {
            size =
                ' | <span style="color: white; border-radius: 5px; background: #5A5A5A; padding: 1px 4px 1px 4px;">' +
                i8ln(item["size"]) +
                "</span>";
        }
    ...
    if (weatherBoostedCondition !== 0) {
        details +=
            '<img style="height:25px;position:absolute;top:25px;left:5px;" src="static/weather/a-' +
            weatherBoostedCondition +
            '.png"></div>';
    }

    var contentstring = "<div><center>" + "<b>" + name + "</b>";
    ...
    contentstring +=
        "<span> - </span>" +
        "<small>" +
        '<a href="https://pokemon.gameinfo.io/' +
        languageSite +
        "/pokemon/" +
        id +
        '" target="_blank" title="' +
        i8ln("View in Pokedex") +
        '">#' +
        id +
        "</a>" +
        "</small>";
    ...
    if (item["expire_timestamp_verified"] > 0) {
        contentstring +=
            ...
            " " +
            getTimeStr(disappearTime) +
            ' <span class="label-countdown" disappears-at="' +
            disappearTime +
            '">(00m00s)</span>' +
            ' <i class="fas fa-check-square" style="color:#28b728;" title="' +
            i8ln("Despawntime verified") +
            '"></i>' +
            "</b></div>";
        ...
        contentstring +=
            ...
            disappearTime +
            '">(00m00s)</span>' +
            ' <i class="fas fa-question" style="color:red;" title="' +
            i8ln("Despawntime not verified") +
            '"></i>' +
            "</b></div>";
    }
    ...
    contentstring +=
        '<a href="javascript:excludePokemon(' +
        id +
        ')"  title="' +
        i8ln("Exclude this Pokémon") +
        '"><i class="fas fa-minus-circle" style="font-size:15px;width:20px;"></i></a>' +
        ' | <a href="javascript:notifyAboutPokemon(' +
        id +
        ')" title="' +
        i8ln("Notify about this Pokémon") +
        '"><i class="fas fa-bell" style="font-size:15px;width:20px;"></i></a>';
    if (!noHideSingleMarker) {
        contentstring +=
            " | <a href=\"javascript:removePokemonMarker('" +
            encounterId +
            '\')" title="' +
            i8ln("Remove this Pokémon from the map") +
            '"><i class="fas fa-eye-slash" style="font-size:15px;width:20px;"></i></a>';
    }
    contentstring +=
        ' | <a href="javascript:void(0);" onclick="javascript:toggleOtherPokemon(' +
        id +
        ');" title="' +
        i8ln("Toggle display of other Pokémon") +
        '"><i class="fas fa-search-plus" style="font-size:15px;width:20px;"></i></a>' +
        "</div></center>";
    if (atk != null && def != null && sta != null && noCatchRates) {
    ...
    contentstring +=
        '<a href="javascript:void(0)" onclick="javascript:openMapDirections(' +
        latitude +
        ", " +
        longitude +
        ')" title="' +
        i8ln("View in Maps") +
        '">' +
        '<i class="fas fa-road" style="padding-right:0.25em"></i>' +
        coordText +
        "</a>";
    if (hidePokemonCoords === true) {
        contentstring += "-";
        ...
        contentstring +=
            " " +
            '<button onclick="copyCoordsToClipboard(this.previousElementSibling);" class="small-tight">' +
            "Copy" +
            "</button> ";
    }
    contentstring +=
    ...
    if (!noPvp) {
        if (
            item["pvp_rankings_little_league"] !== undefined &&
            item["pvp_rankings_little_league"] !== null
        ) {
            contentstring += "<br>";
            ...
            $.each(littleLeague, function (index, ranking) {
                let pokemonName = "";
                $.each(
                    pokedex[ranking.pokemon]["forms"],
                    function (index, form) {
                        if (
                            ranking.form === form["protoform"] &&
                            form["nameform"] !== "Normal"
                        ) {
                            pokemonName =
                                i8ln(form["nameform"]) +
                                " " +
                                i8ln(pokedex[ranking.pokemon]["name"]);
                        }
                    }
                );
                if (pokemonName === "") {
        ...
        if (
            item["pvp_rankings_great_league"] !== undefined &&
            item["pvp_rankings_great_league"] !== null
        ) {
            contentstring += "<br>";
        ...
        if (
            item["pvp_rankings_ultra_league"] !== undefined &&
            item["pvp_rankings_ultra_league"] !== null
        ) {
            contentstring += "<br>";
...
function gymLabel(item) {
    ...
    var numStars =
        item["raid_level"] >= 11 && item["raid_level"] <= 15
            ? item["raid_level"] - 10
            : item["raid_level"];
    var shadowStr =
        (item["raid_level"] >= 11 && item["raid_level"] <= 15) ||
        parseInt(item["raid_pokemon_alignment"]) === 1
            ? i8ln("Shadow") + " "
            : "";
    ...
    if (raidSpawned && item.raid_end > Date.now()) {
        ...
        if (raidStarted) {
            ...
            if (
                form !== null &&
                form > 0 &&
                item["raid_pokemon_form_name"] !== "Normal"
            ) {
        ...
        if (
            raidStarted &&
            item.raid_pokemon_move_1 > 0 &&
            item.raid_pokemon_move_1 !== "133" &&
            item.raid_pokemon_move_2 > 0 &&
            item.raid_pokemon_move_2 !== "133"
        ) {
            var pMove1 =
                moves[item["raid_pokemon_move_1"]] !== undefined
                    ? i8ln(moves[item["raid_pokemon_move_1"]]["name"])
                    : "gen/unknown";
            var pMove2 =
                moves[item["raid_pokemon_move_2"]] !== undefined
                    ? i8ln(moves[item["raid_pokemon_move_2"]]["name"])
                    : "gen/unknown";
        ...
        raidStr +=
            ...
            ": <b>" +
            raidStartStr +
            '</b> <span class="label-countdown" disappears-at="' +
            item["raid_start"] +
            '" start>(' +
            generateRemainingTimer(item["raid_start"], "start") +
            ")</span></div>";
        raidStr +=
            "<div>" +
            i8ln("End") +
        ...
        if (!noHideSingleMarker) {
            raidStr +=
                "<a href=\"javascript:removeGymMarker('" +
                item["gym_id"] +
                '\')" title="' +
                i8ln("Hide this Gym") +
                '"><i class="fas fa-eye-slash" style="font-size:15px;"></i></a>';
        }
        if (raidStarted) {
            raidIcon =
            ...
            raidIcon =
                '<img src="' +
                getIcon(iconpath.raid, "raid/egg", ".png", item["raid_level"]) +
                '" style="height:70px;">';
        }
    }
    if (!noRaids && manualRaids && item["scanArea"] === false) {
        raidStr +=
            ...
            i8ln("Add raid ") +
            '<i class="fas fa-binoculars submit-raid" onclick="openRaidModal(event);" data-id="' +
            item["gym_id"] +
            '"></i>' +
            "</div>";
    }
    if (!noDeleteGyms) {
        raidStr +=
            '<i class="fas fa-trash-alt delete-gym" onclick="deleteGym(event);" data-id="' +
            item["gym_id"] +
            '"></i>';
    }
    if (!noRenameGyms) {
        raidStr +=
            '<center><div><i class="fas fa-edit rename-gym" onclick="openRenameGymModal(event);" data-id="' +
            item["gym_id"] +
            '">' +
            i8ln("Rename Gym") +
            "</i></div></center>";
    ...
    if (!noToggleExGyms) {
        raidStr +=
            '<i class="fas fa-trophy toggle-ex-gym" onclick="toggleExGym(event);" data-id="' +
            item["gym_id"] +
            '"></i>';
    }
    ...
    var lastScannedStr =
        lastScanned != null
            ? "<div>" +
              i8ln("Last Scanned") +
              ": " +
              getDateStr(lastScanned) +
              " " +
              getTimeStr(lastScanned) +
              "</div>"
            : "";

    var lastModifiedStr =
        getDateStr(lastModified) + " " + getTimeStr(lastModified);
    ...
    if (url !== null) {
        gymImage =
            '<img id="' +
            item["gym_id"] +
            '"class="gym-image" style="border:3px solid rgba(' +
            gymColor[teamId] +
            ')" src="' +
            url +
            '" onclick="openFullscreenModal(document.getElementById(\'' +
            item["gym_id"] +
            "').src)\">";
    }
    var inBattle =
        item["in_battle"] === 1 && lastScanned > Date.now() - 5 * 60 * 1000
            ? '<img src="static/images/in_battle_small.png" style="position:absolute;right:170px;bottom:140px;"/>'
            : "";
    var teamStr =
        teamId === 0
    ...
    if (
        !noWhatsappLink &&
        raidSpawned &&
        item.raid_end > Date.now() &&
        item.raid_pokemon_id > 1 &&
        item.raid_pokemon_id < numberOfPokemon
    ) {
        whatsappLink =
            ...
            item.raid_level +
            "%20" +
            item.raid_pokemon_name +
            "%0A%2AStart:%20" +
            raidStartStr +
            "%2A%0A%2AEnd:%20" +
            raidEndStr +
            "%2A%0AStats:%0Ahttps://pokemongo.gamepress.gg/pokemon/" +
            item.raid_pokemon_id +
            "%0AMoves:%0A" +
            pMove1 +
            " / " +
            pMove2 +
            "%0A%0ADirections:%0Ahttps://www.google.com/maps/search/?api=1%26query=" +
            item.latitude +
            "," +
            item.longitude +
            '" data-action="share/whatsapp/share"><i class="fab fa-whatsapp" style="position:relative;top:3px;left:5px;color:#26c300;font-size:20px;"></i></a>';
        ...
        whatsappLink =
            ...
            "%2A%0A%2AEnd:%20" +
            raidEndStr +
            "%2A%0ADirections:%0Ahttps://www.google.com/maps/search/?api=1%26query=" +
            item.latitude +
            "," +
            item.longitude +
            '" data-action="share/whatsapp/share"><i class="fab fa-whatsapp" style="position:relative;top:3px;left:5px;color:#26c300;font-size:20px;"></i></a>';
    ...
    str +=
        ...
        "&zoom=18&gymId=" +
        item["gym_id"] +
        '"><i class="far fa-share-square" aria-hidden="true" style="position:relative;top:3px;left:0px;color:#26c300;font-size:20px;"></i></a>' +
        whatsappLink +
        "</div>" +
        "<div>" +
        i8ln("Last Modified") +
...
function getReward(item) {
    ...
    if (!noTeamRocket && item["incident_expiration"] > Date.now()) {
        ...
        styleStr =
            "position:absolute;height:35px;right:55%;top:85px;filter:drop-shadow(1px 0 0 black)drop-shadow(-1px 0 0 black);";
    } else {
    ...
    if (mapFork === "mad") {
        ...
        if (item["quest_condition_type"] > 0) {
            switch (item["quest_condition_type"]) {
                case 1:
                    ...
                    if (questinfo["pokemon_type_ids"].length > 1) {
                        $.each(
                            questinfo["pokemon_type_ids"],
                            function (index, typeId) {
                                if (
                                    index ===
                                    questinfo["pokemon_type_ids"].length - 2
                                ) {
                                    tstr += i8ln(pokemonTypes[typeId]) + " or ";
                                } else if (
                                    index ===
                                    questinfo["pokemon_type_ids"].length - 1
                                ) {
                        ...
                        tstr = i8ln(
                            pokemonTypes[questinfo["pokemon_type_ids"]]
                        );
                    }
                    if (item["quest_condition_type_1"] === 21) {
                        str = str.replace(
                            "Catch {0}",
                            "Catch {0} different species of"
                        );
                    }
                    str = str.replace("pokémon", tstr + "-type Pokémon");
                    str = str.replace(
                        "Snapshot(s)",
                        "Snapshot(s) of " + tstr + "-type Pokémon"
                    );
                    break;
                case 2:
                    var pstr = "";
                    if (questinfo["pokemon_ids"].length > 1) {
                        $.each(questinfo["pokemon_ids"], function (index, id) {
                            if (index === questinfo["pokemon_ids"].length - 2) {
                                ...
                                index ===
                                questinfo["pokemon_ids"].length - 1
                            ) {
                                pstr += idToPokemon[id];
                            } else {
                                pstr += idToPokemon[id] + ", ";
                ...
                case 7:
                    ...
                    if (raidLevel > 1) {
                        str = str.replace(
                            "raid battle(s)",
                            "level " + raidLevel + " or higher raid"
                        );
                    }
                ...
                case 8:
                    ...
                    str = str.replace(
                        "throw(s)",
                        throwType[questinfo["throw_type_id"]] + " Throw(s)"
                    );
                ...
                case 10:
                    if (item["quest_type"] === 53) {
                        ...
                        str = str.replace(
                            "Complete",
                            "Use a super effective charged attack in"
                        );
                    }
                    break;
                case 11:
                    if (item["quest_type"] === 13) {
                        str = str
                            .replace("Catch", "Use")
                            .replace(
                                "pokémon with berrie(s)",
                                "berrie(s) to help catch Pokémon"
                            );
                    }
                    if (questinfo !== null) {
                        str = str.replace(
                            "berrie(s)",
                            idToItem[questinfo["item_id"]]
                        );
                        str = str.replace(
                            "Evolve {0} pokémon",
                            "Evolve {0} pokémon with a " +
                                idToItem[questinfo["item_id"]]
                        );
                    } else {
                ...
                case 12:
                    str = str.replace(
                        "pokéstop(s)",
                        "pokéstop(s) you haven't visited before"
                    );
                    break;
                case 14:
                    str = str.replace("Land", "Make");
                    if (typeof questinfo["throw_type_id"] === "undefined") {
                        ...
                        str = str.replace(
                            "throw(s)",
                            throwType[questinfo["throw_type_id"]] +
                                " Throw(s) in a row"
                        );
                    }
                    if (item["quest_condition_type_1"] === 15) {
                ...
                case 22:
                    str = str
                        .replace("Win", "Battle a Team Leader")
                        .replace("pvp battle(s)", "times");
                    break;
                case 23:
                    str = str
                        .replace("Win", "Battle Another Trainer")
                        .replace("pvp battle(s)", "times");
                    break;
                case 25:
                    str = str.replace(
                        "{0} pokémon",
                        "pokémon caught " + questinfo["distance"] + "km apart"
                    );
                    break;
                case 26:
                ...
                case 27:
                    var gstr = "";
                    $.each(
                        questinfo["character_category_ids"],
                        function (index, charId) {
                            if (
                                index ===
                                questinfo["character_category_ids"].length - 2
                            ) {
                                gstr += gruntCharacterTypes[charId] + " or ";
                            } else if (
                                index ===
                                questinfo["character_category_ids"].length - 1
                            ) {
                ...
                case 41:
                    if (
                        item["quest_type"] === 27 &&
                        questinfo !== null &&
                        parseInt(questinfo["combat_type"]) === 6
                    ) {
            ...
            switch (item["quest_type"]) {
                case 7:
                    str = str
                        .replace("Complete", "Battle in a gym")
                        .replace("gym battle(s)", "times");
                    break;
                case 8:
                    str = str
                        .replace("Complete", "Battle in a raid")
                        .replace("raid battle(s)", "times");
                    break;
                case 13:
                    str = str
                        .replace("Catch", "Use")
                ...
                case 17:
                    str = str
                        .replace("Walk your buddy to earn", "Earn")
                        .replace("candy", "candy walking with your buddy");
                    break;
            }
        }
...
function pokestopLabel(item) {
    ...
    if (!noTeamRocket && item["incident_expiration"] > Date.now()) {
        stopName =
            '<b class="pokestop-rocket-name">' + item["pokestop_name"] + "</b>";
        if (item["url"] !== null) {
            stopImage =
                '<img class="pokestop-rocket-image" id="' +
                item["pokestop_id"] +
                '" src="' +
                item["url"] +
                '" onclick="openFullscreenModal(document.getElementById(\'' +
                ...
                '" style="position:absolute;height:35px;right:55%;top:85px;">';
        }
    } else if (!noEventStops && item["eventstops_expiration"] > Date.now()) {
        stopName =
            '<b class="pokestop-eventstops-name">' +
            item["pokestop_name"] +
            "</b>";
        if (item["url"] !== null) {
            stopImage =
                '<img class="pokestop-eventstops-image" id="' +
                item["pokestop_id"] +
                '" src="' +
                item["url"] +
                '" onclick="openFullscreenModal(document.getElementById(\'' +
        ...
        !noQuests &&
        item["quest_type"] > 0 &&
        lastMidnight < Number(item["quest_timestamp"])
    ) {
        stopName =
            '<b class="pokestop-quest-name">' + item["pokestop_name"] + "</b>";
        if (item["url"] !== null) {
            stopImage =
                '<img class="pokestop-quest-image" id="' +
                item["pokestop_id"] +
                '" src="' +
                item["url"] +
                '" onclick="openFullscreenModal(document.getElementById(\'' +
        ...
        stopName =
            '<b class="pokestop-lure-name">' + item["pokestop_name"] + "</b>";
        if (item["url"] !== null) {
            stopImage =
                '<img class="pokestop-lure-image" id="' +
                item["pokestop_id"] +
                '" src="' +
                item["url"] +
                '" onclick="openFullscreenModal(document.getElementById(\'' +
    ...
    if (
        !noQuests &&
        item["quest_type"] > 0 &&
        typeof questtypeList[item["quest_type"]] !== "undefined" &&
        lastMidnight < Number(item["quest_timestamp"])
    ) {
        ...
        if (item["quest_with_artask"] === true) {
            questArStr =
                '<div><span class="pokestop-quest-artext">' +
                i8ln("With AR-Scan Task") +
                "</span></div>";
        } else if (item["quest_with_artask"] === false) {
            questArStr =
                '<div><span class="pokestop-quest-artext">' +
                i8ln("Without AR-Scan Task") +
                "</span></div>";
        }
        str +=
            getReward(item) +
        ...
        if (!noHideSingleMarker) {
            str +=
                "<a href=\"javascript:removePokestopMarker('" +
                item["pokestop_id"] +
                '\')" title="' +
                i8ln("Hide this Pokéstop") +
                '"><i class="fas fa-eye-slash" style="font-size:15px;"></i></a>';
        }
    } else {
        str += "</div>";
    ...
    if (
        ...
        str +=
            "<div>" +
            i8ln("Error: Undefined Quest Type") +
            ": " +
            item["quest_type"] +
            "</div>";
    }
    if (!noEventStops && item["eventstops_expiration"] > Date.now()) {
        ...
        if (item["eventstops_id"] === 7) {
            eventType =
                '<img src="static/sprites/misc/EventStopsCoin.png" style="padding:5px;position:relative;left:0px;top:12px;height:40px;"/>';
        } else if (item["eventstops_id"] === 8) {
            eventType =
                '<img src="' +
                getIcon(iconpath.pokemon, "pokemon", ".png", 352) +
                '" style="padding:5px;position:relative;left:0px;top:12px;height:40px;"/>';
        } else if (item["eventstops_id"] === 9) {
            eventType = i8ln("Showcase");
        } else {
            console.log("Unknown Event Type: " + item["eventstops_id"]);
            eventType =
                i8ln("Unknown Event Type") + ": " + item["eventstops_id"];
        }
        ...
        str +=
            ...
            "</b></div>" +
            "<div>" +
            i8ln("Event Expiration") +
            ": <b>" +
            eventEndStr +
            ' <span class="label-countdown" disappears-at="' +
            item["eventstops_expiration"] +
            '">(00m00s)</span>' +
            "</b></div>";
    }
    if (!noLures && item["lure_expiration"] > Date.now()) {
        var lureType =
            '<img style="padding:5px;position:relative;left:0px;top:12px;height:40px;" src="static/sprites/misc/LureModule_' +
            item["lure_id"] +
            '.png"/>';
        if (item["lure_id"] === 501) {
    ...
    if (!noTeamRocket && item["incident_expiration"] > Date.now()) {
        ...
        str +=
            "<div>" +
            i8ln("Expiration Time") +
            ": <b>" +
            incidentEndStr +
            ' <span class="label-countdown" disappears-at="' +
            item["incident_expiration"] +
            '">(00m00s)</span>' +
            "</b></div>";
        if (!noInvasionEncounterData && item["encounters"] !== null) {
            gruntReward +=
                '<input class="button" name="button" type="button" onclick="showHideGruntEncounter()" value="' +
                (item["encounters_confirmed"] == 1
                    ? i8ln("Show / Hide Confirmed Rewards")
                    : i8ln("Show / Hide Possible Rewards")) +
                '" style="margin-top:2px;outline:none;font-size:9pt">' +
            ...
            if (item["second_reward"] === "false") {
                ...
                item["encounters"]["first"].forEach(function (pokemonid) {
                    gruntReward +=
                        ...
                        '" style="width:30px;height:auto;position:absolute;"/>' +
    ...
    if (!noDeletePokestops) {
        str +=
            '<i class="fas fa-trash-alt delete-pokestop" onclick="deletePokestop(event);" data-id="' +
            item["pokestop_id"] +
            '"></i>';
    }
    if (!noManualQuests && item["scanArea"] === false) {
        str +=
            '<center><div><i class="fas fa-binoculars submit-quest" onclick="openQuestModal(event);" data-id="' +
            item["pokestop_id"] +
            '">' +
            i8ln("Add Quest") +
            "</i></div></center>";
    ...
    if (!noRenamePokestops) {
        str +=
            '<center><div><i class="fas fa-edit rename-pokestop" onclick="openRenamePokestopModal(event);" data-id="' +
            item["pokestop_id"] +
            '">' +
            i8ln("Rename Pokestop") +
            "</i></div></center>";
    }
    if (!noConvertPokestops) {
        str +=
            '<center><div><i class="fas fa-sync-alt convert-pokestop" onclick="openConvertPokestopModal(event);" data-id="' +
            item["pokestop_id"] +
            '">' +
            i8ln("Convert to Gym") +
            "</i></div></center>";
    ...
    if (
        ...
        str +=
            '<a href="whatsapp://send?text=' +
            encodeURIComponent(item["pokestop_name"]) +
            "%0A%2AQuest:%20" +
            quest +
            "%2A%0A%2AReward:%20" +
            reward +
            "%2A%0Ahttps://www.google.com/maps/search/?api=1%26query=" +
            item["latitude"] +
            "," +
            item["longitude"] +
    ...
    if (
        ...
        str +=
            ...
            i8ln("Quest found") +
            ": " +
            getDateStr(item["quest_timestamp"] * 1000) +
            " " +
            getTimeStr(item["quest_timestamp"] * 1000) +
            "</div></center>";
    }
    str +=
        "<center><div>" +
        i8ln("Last seen") +
        ": " +
        getDateStr(item["last_seen"]) +
        " " +
        getTimeStr(item["last_seen"]) +
        "</div></center>";
    return str;
}
...
function formatSpawnTime(seconds) {
    return (
        ("0" + Math.floor(((seconds + 3600) % 3600) / 60)).substr(-2) +
        ":" +
...
function getNotifyText(item) {
    ...
    var replace = [
        ...
        item["individual_defense"],
        item["individual_stamina"],
    ];
...
function customizePokemonMarker(marker, item, skipNotification) {
    ...
    if (
        ...
        if (!skipNotification) {
            ...
            sendNotification(
                getNotifyText(item).fav_title,
                getNotifyText(item).fav_text,
                getIcon(
                    iconpath.pokemon,
                    "pokemon",
                    ".png",
        ...
        if (
            marker.animationDisabled !== true &&
            Store.get("remember_bounce_notify")
        ) {
            marker.bounce();
        }
    }
...
function getGymMarkerIcon(item) {
    ...
    var fortMarker = "";
    var exIcon =
        ((park !== "0" && onlyTriggerGyms === false && park) ||
            triggerGyms.includes(item["gym_id"])) &&
        noExGyms === false
            ? '<img src="static/images/ex.png" style="position:absolute;right:25px;bottom:2px;"/>'
            : "";
    var inBattle =
        item["in_battle"] === 1 &&
        item.last_scanned > Date.now() - 5 * 60 * 1000
            ? '<img src="static/images/in_battle_small.png" style="width:26px;position:absolute;right:31px;bottom:28px;"/>'
            : "";
    var smallExIcon =
        ((park !== "0" && onlyTriggerGyms === false && park) ||
            triggerGyms.includes(item["gym_id"])) &&
        noExGyms === false
            ? '<img src="static/images/ex.png" style="width:26px;position:absolute;right:35px;bottom:13px;"/>'
            : "";
    var html = "";
    if (item["raid_pokemon_id"] != null && item.raid_end > Date.now()) {
        html =
            ...
            '" style="width:50px;height:auto;position:absolute;top:-15px;right:0px;"/>' +
            "</div>";
        if (noRaidTimer === false && Store.get(["showRaidTimer"])) {
            html +=
                '<div class="gym-icon-raid-timer"><span class="icon-countdown" style="padding: .25rem!important; white-space: nowrap;" disappears-at="' +
                item["raid_end"] +
                '" end>' +
                generateRemainingTimer(item["raid_end"], "end") +
                "</span></div>";
        ...
        item["raid_level"] !== null &&
        item.raid_start <= Date.now() &&
        item.raid_end > Date.now()
    ) {
        html =
            '<div style="position:relative;">' +
            ...
            inBattle +
            '<img src="' +
            getIcon(iconpath.raid, "raid/egg", ".png", item["raid_level"], 1) +
            '" style="width:35px;height:auto;position:absolute;top:-11px;right:18px;"/>' +
            "</div>";
        if (noRaidTimer === false && Store.get(["showRaidTimer"])) {
            html +=
                '<div class="gym-icon-raid-timer"><span class="icon-countdown" style="padding: .25rem!important; white-space: nowrap;" disappears-at="' +
        ...
        html =
            ...
            '<img src="' +
            getIcon(iconpath.raid, "raid/egg", ".png", item["raid_level"]) +
            '" style="width:30px;position:absolute;top:4px;right:15px;"/>' +
            "</div>";
        if (noRaidTimer === false && Store.get(["showRaidTimer"])) {
            html +=
                '<div class="gym-icon-egg-timer"><span class="icon-countdown" style="padding: .25rem!important; white-space: nowrap;" disappears-at="' +
                item["raid_start"] +
                '" end>' +
...
function setupGymMarker(item) {
    ...
    if (!marker.rangeCircle && isRangeActive(map)) {
        marker.rangeCircle = addRangeCircle(
            marker,
            map,
            "gym",
            item["team_id"]
    ...
    if (
        raidLevel >= Store.get("remember_raid_notify") &&
        item.raid_end > Date.now() &&
        Store.get("remember_raid_notify") !== 0
    ) {
...
function updateGymIcons() {
    $.each(mapData.gyms, function (key, value) {
        mapData.gyms[key]["marker"].setIcon(
            getGymMarkerIcon(mapData.gyms[key])
        );
        mapData.gyms[key]["marker"].setPopupContent(
            gymLabel(mapData.gyms[key])
        );
    });
}
...
function updatePokestopIcons() {
    $.each(mapData.pokestops, function (key, value) {
        mapData.pokestops[key]["marker"].setIcon(
            getPokestopMarkerIcon(mapData.pokestops[key])
        );
        mapData.pokestops[key]["marker"].setPopupContent(
            pokestopLabel(mapData.pokestops[key])
        );
    });
}
...
function getPokestopMarkerIcon(item) {
    ...
    if (
        Store.get(["showPokestops"]) &&
        !Store.get(["showQuests"]) &&
        !Store.get(["showLures"]) &&
        !Store.get(["showRocket"]) &&
        !Store.get(["showEventStops"]) &&
        !Store.get(["showAllPokestops"])
    ) {
        stopMarker = L.divIcon({
            ...
            html:
                '<div><img src="' +
                getIcon(iconpath.pokestop, "pokestop", ".png", markerStr) +
                '" style="width:50px;height:72;top:-35px;right:10px;"/></div>',
        });
        ...
        if (!noTeamRocket && item["incident_expiration"] > Date.now()) {
            ...
            html =
                ...
                getIcon(iconpath.pokestop, "pokestop", ".png", markerStr, 1) +
                '" style="width:50px;height:72;top:-35px;right:10px;"/>';
            if (item["grunt_type"] > 0) {
                html +=
                    '<img src="' +
                    getIcon(
                    ...
                    '" style="width:30px;height:auto;position:absolute;top:4px;left:0px;"/></div>';
            } else {
                html += "</div>";
            }
            if (noRocketTimer === false && Store.get(["showRocketTimer"])) {
                html +=
                    '<div class="pokestop-icon-rocket-timer"><span class="icon-countdown" style="padding: .25rem!important; white-space: nowrap;" disappears-at="' +
                    item["incident_expiration"] +
                    '"> </span></div>';
            }
            stopMarker = L.divIcon({
            ...
            !noEventStops &&
            item["eventstops_expiration"] > Date.now()
        ) {
            if (!noLures && item["lure_expiration"] > Date.now()) {
                markerStr = item["lure_id"];
            }
            ...
            if (item["eventstops_id"] === 7) {
                html +=
                    '<img src="static/sprites/misc/EventStopsCoin.png" style="width:25px;height:auto;position:absolute;top:4px;left:0px;"/></div>';
            } else if (item["eventstops_id"] === 8) {
                html +=
                    '<img src="' +
                    getIcon(iconpath.pokemon, "pokemon", ".png", 352) +
                    '" style="width:25px;height:auto;position:absolute;top:4px;left:0px;"/></div>';
            } else {
                html += "</div>";
            }
            if (
                noEventStopsTimer === false &&
                Store.get(["showEventStopsTimer"])
            ) {
                html +=
                    '<div class="pokestop-icon-eventstops-timer"><span class="icon-countdown" style="padding: .25rem!important; white-space: nowrap;" disappears-at="' +
                    item["eventstops_expiration"] +
                    '"> </span></div>';
            }
            stopMarker = L.divIcon({
            ...
            if (item["quest_reward_type"] === 12) {
                html =
                    ...
                    '" style="width:30px;height:auto;position:absolute;top:4px;left:0px;"/>' +
                    "</div>";
                stopMarker = L.divIcon({
                    iconSize: [31, 31],
                    iconAnchor: [25, 45],
                ...
                html =
                    ...
                    '" style="width:25px;height:auto;position:absolute;top:6px;left:10px;"/>' +
                    "</div>";
                stopMarker = L.divIcon({
                    iconSize: [31, 31],
                    iconAnchor: [25, 45],
        ...
        Store.get(["showRocket"]) &&
        !noTeamRocket &&
        item["incident_expiration"] > Date.now()
    ) {
        if (!noLures && item["lure_expiration"] > Date.now()) {
        ...
        Store.get(["showLures"]) &&
        !noLures &&
        item["lure_expiration"] > Date.now()
    ) {
        html =
            '<div><img src="' +
            getIcon(iconpath.pokestop, "pokestop", ".png", item["lure_id"]) +
            '" style="width:50px;height:72;top:-35px;right:10px;"/><div>';
        stopMarker = L.divIcon({
            iconSize: [31, 31],
            iconAnchor: [25, 45],
...
function setupNestMarker(item) {
    ...
    if (item.pokemon_id > 0) {
        ...
        getNestMarkerIcon =
            ...
            getIcon(
                ...
                "nest",
                ".png",
                getKeyByValue(pokemonTypes, item.english_pokemon_types[0].type)
            ) +
            '" style="width:45px;height: auto;"/>' +
            '<img src="' +
            getIcon(iconpath.pokemon, "pokemon", ".png", pokemonId, 0, formId) +
            '" style="position:absolute;width:40px;height:40px;top:6px;left:3px"/>' +
            "</div>";
    } else {
        getNestMarkerIcon =
            '<div class="marker-nests">' +
            '<img src="' +
            getIcon(iconpath.nest, "nest", ".png", 0) +
    ...
    if (
        noNestPolygon === false &&
        Store.get("showNestPolygon") === true &&
        item["polygon_path"] !== null
    ) {
        var polygonColor = item["pokemon_types"][0]["color"]
            ? item["pokemon_types"][0]["color"]
            : "grey";
...
function nestLabel(item) {
    ...
    if (item.pokemon_id > 0) {
        ...
        $.each(types, function (index, type) {
            typesDisplay +=
                ...
                '" style="height:20px;top:5px;position:relative;">';
        });
        var formId = item.pokemon_form;
        var pokemonId = item.pokemon_id;
        var nestName =
            item["name"] !== null && item["name"] !== "Unknown Areaname"
                ? "<b>" + item["name"] + "</b>"
                : "";
        var pokemonAvg =
            item["pokemon_avg"] > 0
                ? "<div>" +
                  i8ln("Nest Pokemon per hour") +
                  ": " +
                  item["pokemon_avg"].toFixed(2) +
                  "</div>"
                : "";
        var pokemonCount =
            item["pokemon_count"] > 0
                ? "<div>" +
                  i8ln("Total nest Pokemon count") +
                  ": " +
                  item["pokemon_count"] +
                  "</div>"
                : "";
        var nestSubmittedBy =
            item["nest_submitted_by"] !== null
                ? "<div>" +
                  i8ln("Submitted by") +
                  ": " +
                  item["nest_submitted_by"] +
                  "</div>"
                : "";
        ...
        str +=
            '<div align="center" class="marker-nests">' +
            '<img src="' +
            getIcon(iconpath.nest, "nest", ".png", 0) +
            '" align"middle" style="width:36px;height: auto;"/>' +
            "</div>" +
            "<center><b>" +
            i8ln("No Pokemon - Assign One Below") +
            "</b></center>";
    }
    if (item.type === 0) {
        str +=
            '<center><div style="margin-bottom:5px; margin-top:5px;">' +
            i8ln("Found by ") +
            nestBotName +
            "</div></center>";
    }
    if (!noDeleteNests) {
        str +=
            '<i class="fas fa-trash-alt delete-nest" onclick="deleteNest(event);" data-id="' +
            item["nest_id"] +
            '"></i>';
    }
    if (!noManualNests) {
        str +=
            "<center><div>" +
            i8ln("Add Nest") +
            ' <i class="fas fa-binoculars submit-nest" onclick="openNestModal(event);" data-id="' +
            item["nest_id"] +
            '"></i></div></center>';
    }
    ...
    str +=
        ...
        "&lon=" +
        item.lon +
        '&zoom=18"><i class="far fa-share-square" aria-hidden="true" style="position:relative;top:3px;left:0px;color:#26c300;font-size:20px;"></i></a>' +
        "</div></center>";
    if (!noWhatsappLink && item.pokemon_id > 0) {
        str +=
            "<div>" +
            "<center>" +
            '<a href="whatsapp://send?text=%2A' +
            encodeURIComponent(item.pokemon_name) +
            "%2A%20nest has been found.%0A%0ALocation:%20https://www.google.com/maps/search/?api=1%26query=" +
            item.lat +
            "," +
            item.lon +
            '" data-action="share/whatsapp/share">' +
    ...
    str +=
        ...
        i8ln("Last Updated") +
        ": " +
        getDateStr(item["updated"]) +
        " " +
        getTimeStr(item["updated"]) +
        "</div></center>";
    return str;
}
...
function setupCommunityMarker(item) {
    var icon = L.divIcon({
        ...
        html:
            '<img src="static/images/marker-' +
            item.type +
            '.png" style="width:36px;height: auto;"/>',
    });
...
function communityLabel(item) {
    var str =
        '<div align="center" class="marker-community">' +
        '<img src="static/images/marker-' +
        item.type +
        '.png" align"middle" style="width:30px;height: auto;"/>';
    ...
    if (
        item.has_invite_url === 1 &&
        (item.invite_url !== "#" || item.invite_url !== undefined)
    ) {
        str +=
            '<button class="btn btn-primary btn-sm" onclick="window.open(\'' +
            item.invite_url +
            '\',\'_blank\')" type="button"><i class="fas fa-comments"></i> ' +
            i8ln("Join Now") +
            "</button>";
    }
    if (!noEditCommunity) {
        str +=
            '<button class="btn btn-primary btn-sm" role="button" onclick="openEditCommunityModal(event);" data-id="' +
            item.community_id +
            '" data-title="' +
            item.title +
            '" data-description="' +
    ...
    if (item.source === 2) {
        str +=
            '<center><div style="margin-bottom:5px; margin-top:5px;">' +
            i8ln("Join on") +
            '<a href="https://thesilphroad.com/map#18/' +
            item.lat +
            "/" +
            item.lon +
            '">thesilphroad.com</a>' +
    ...
    if (!noDeleteCommunity) {
        str +=
            '<i class="fas fa-trash-alt delete-community" onclick="deleteCommunity(event);" data-id="' +
            item.community_id +
            '"></i>';
    }
    return str;
...
function setupPortalMarker(item) {
    ...
    var marker = L.circle([item["lat"], item["lon"]], circle).bindPopup(
        portalLabel(item),
...
function setupPoiMarker(item) {
    ...
    var html =
        '<div><span class="' +
        dot +
        '" style="width:20px;height:20px;"></span></div>';
...
function portalLabel(item) {
    ...
    var str =
        '<center><div style="font-weight:900;font-size:12px;margin-left:10px;">' +
        item.name +
        "</div></center>" +
        '<center><img id="' +
        item.external_id +
        '" src="' +
        item.url +
        '" align"middle" style="width:175px;height:auto;" onclick="openFullscreenModal(document.getElementById(\'' +
        item.external_id +
        "').src)\"/></center>";
    if (!noConvertPortal) {
        str +=
            '<div class="d-grid">' +
            '<button class="btn btn-primary btn-sm" role="button" onclick="openConvertPortalModal(event);" data-id="' +
            item.external_id +
            '"><i class="fas fa-sync-alt convert-portal"></i>' +
            " " +
            i8ln("Convert portal") +
            "</button>" +
            "</div>";
    ...
    if (!noDeletePortal) {
        str +=
            '<i class="fas fa-trash-alt delete-portal" onclick="deletePortal(event);" data-id="' +
            item.external_id +
            '"></i>';
    }
    return str;
...
function poiLabel(item) {
    ...
    if (item.status === "1") {
        dot = "dot possible-candidate";
        str +=
            '<center><div style="font-weight:900;margin-bottom:5px;">' +
            i8ln("Possible Candidate") +
            "</div></center>";
    } else if (item.status === "2") {
        dot = "dot candidate-submitted";
        str +=
            '<center><div style="font-weight:900;margin-bottom:5px;">' +
            i8ln("Candidate submitted") +
            "</div></center>";
    } else if (item.status === "3") {
        dot = "dot candidate-declined";
        str +=
            '<center><div style="font-weight:900;margin-bottom:5px;">' +
            i8ln("Candidate declined") +
            "</div></center>";
    } else if (item.status === "4") {
        dot = "dot candidate-resubmit";
        str +=
            '<center><div style="font-weight:900;margin-bottom:5px;">' +
            i8ln("Candidate eligible for resubmit") +
            "</div></center>";
    } else if (item.status === "5") {
        dot = "dot candidate-not-eligible";
        str +=
            '<center><div style="font-weight:900;margin-bottom:5px;">' +
            i8ln("Not a eligible candidate") +
            "</div></center>";
    }
    str +=
        "<center><div><b>" +
    ...
    if (item.poiimageurl && item.poisurroundingurl) {
        str +=
            '<center><img id="poi-image"src="' +
            item.poiimageurl +
            '" style="float:left;width:45%;margin-right:1%;margin-bottom:0.5em;" onclick="openFullscreenModal(document.getElementById(\'poi-image\').src)"/></center>';
        str +=
            '<center><img id="poi-surrounding" src="' +
            item.poisurroundingurl +
            '" style="float:right;width:45%;margin-right:1%;margin-bottom:0.5em;" onclick="openFullscreenModal(document.getElementById(\'poi-surrounding\').src)"/></center>';
    }
    if (item.poiimageurl && !item.poisurroundingurl) {
        str +=
            '<center><img id="poi-image"src="' +
            item.poiimageurl +
            '" style="width:45%;margin-right:1%;margin-bottom:0.5em;" onclick="openFullscreenModal(document.getElementById(\'poi-image\').src)"/></center>';
    }
    if (item.poisurroundingurl && !item.poiimageurl) {
        str +=
            '<center><img id="poi-surrounding" src="' +
            item.poisurroundingurl +
            '" style="width:45%;margin-right:1%;margin-bottom:0.5em;" onclick="openFullscreenModal(document.getElementById(\'poi-surrounding\').src)"/></center>';
    }
    if (item.poiimageurl || item.poisurroundingurl) {
    ...
    if (!noDeletePoi) {
        str +=
            '<i class="fas fa-trash-alt delete-poi" onclick="deletePoi(event);" data-id="' +
            item.poi_id +
            '"></i>';
    }
    str += '<div class="d-grid">';
    if (!noEditPoi) {
        str +=
            '<button class="btn btn-primary btn-sm" role="button" onclick="openEditPoiModal(event);" data-id="' +
            item.poi_id +
            '" data-name="' +
            item.name +
            '" data-description="' +
            ...
            '" data-poisurrounding="' +
            item.poisurroundingurl +
            '"><i class="fas fa-edit edit-poi"></i> ' +
            i8ln("Edit POI") +
            "</button>";
    }
    if (!noMarkPoi) {
        str +=
            '<button class="btn btn-primary btn-sm" role="button" onclick="openMarkPoiModal(event);" data-id="' +
            item.poi_id +
            '"><i class="fas fa-sync-alt convert-poi"></i> ' +
            i8ln("Mark POI") +
            "</button>";
    }
    str += "</div>";
    str +=
        '<center><a href="javascript:void(0);" onclick="javascript:openMapDirections(' +
        item.lat +
        "," +
        item.lon +
        ');" title="' +
        ...
        "&lon=" +
        item.lon +
        '&zoom=18"><i class="far fa-share-square" aria-hidden="true" style="position:relative;top:3px;left:0px;color:#26c300;font-size:20px;"></i></a></center>';
    return str;
}

function deletePortal(event) {
    // eslint-disable-line no-unused-vars
    ...
    if (portalid && portalid !== "") {
        if (
            confirm(
                i8ln(
                    "I confirm that this portal does not longer exist. This is a permanent removal"
                )
            )
        ) {
            return $.ajax({
                ...
                error: function error() {
                    ...
                    sendToast(
                        "danger",
                        i8ln("Error Deleting portal"),
                        i8ln("Oops something went wrong."),
                        "true"
                    );
                },
                complete: function complete() {
                    sendToast(
                        "success",
                        i8ln("Deleting portal"),
                        i8ln("Deleting portal successful."),
                        "true"
                    );
...
function deletePoi(event) {
    ...
    if (poiid && poiid !== "") {
        if (
            confirm(
                i8ln(
                    "I confirm that this poi has been accepted through niantic or is not eligible as POI. This is a permanent removal"
                )
            )
        ) {
            return $.ajax({
...
function setupScanLocationMarker(item) {
    ...
    if (
        item["last_seen"] <
        Math.round(new Date().getTime() / 1000) - deviceOfflineAfterSeconds
    ) {
        html =
            '<img src="static/images/device-offline.png" style="width:36px;height: auto;"/>';
    } else {
        html =
            '<img src="static/images/device-online.png" style="width:36px;height: auto;"/>';
    }
    var icon = L.divIcon({
        iconSize: [36, 48],
        iconAnchor: [18, 24],
...
function clearStaleMarkers() {
    $.each(mapData.pokemons, function (key, value) {
        ...
        if (Store.get("showMissingIVOnly") === true) {
            keepMons =
                mapData.pokemons[key]["individual_attack"] === null ||
                mapData.pokemons[key]["individual_defense"] === null ||
                mapData.pokemons[key]["individual_stamina"] === null;
        } else {
            keepMons =
                minIV === 0 &&
                minLevel === 0 &&
            ...
            if (!keepMons) {
                if (
                    ...
                    if (
                        Store.get("showZeroIv") === true &&
                        mapData.pokemons[key]["individual_attack"] === 0 &&
                        mapData.pokemons[key]["individual_defense"] === 0 &&
                        mapData.pokemons[key]["individual_stamina"] === 0
                    ) {
                        keepMons = true;
                    } else if (
                        Store.get("showHundoIv") === true &&
                        mapData.pokemons[key]["individual_attack"] === 15 &&
                        mapData.pokemons[key]["individual_defense"] === 15 &&
                        mapData.pokemons[key]["individual_stamina"] === 15
                    ) {
                        keepMons = true;
                    }
                }
                ...
                if (!keepMons) {
                    if (mapData.pokemons[key]["size"] !== null) {
                        if (
                            Store.get("showXXS") === true &&
                            mapData.pokemons[key]["size"] === "XXS"
                        ) {
                            keepMons = true;
                        } else if (
                            Store.get("showXXL") === true &&
                            mapData.pokemons[key]["size"] === "XXL"
                        ) {
                            keepMons = true;
                        }
                    }
                    ...
                    if (!keepMons) {
                        ...
                        if (minLLRank > 0 || minGLRank > 0 || minULRank > 0) {
                            ...
                            if (
                                minLLRank > 0 &&
                                mapData.pokemons[key][
                                    "pvp_rankings_little_league_best"
                                ] !== null &&
                                mapData.pokemons[key][
                                    "pvp_rankings_little_league_best"
                                ] <= minLLRank
                                ...
                                minGLRank > 0 &&
                                mapData.pokemons[key][
                                    "pvp_rankings_great_league_best"
                                ] !== null &&
                                mapData.pokemons[key][
                                    "pvp_rankings_great_league_best"
                                ] <= minGLRank
                                ...
                                minULRank > 0 &&
                                mapData.pokemons[key][
                                    "pvp_rankings_ultra_league_best"
                                ] !== null &&
                                mapData.pokemons[key][
                                    "pvp_rankings_ultra_league_best"
                                ] <= minULRank
                        ...
                        if (!keepMons) {
                            var keepMinIvLvl =
                                excludedMinIV.includes(
                                    mapData.pokemons[key]["pokemon_id"]
                                ) === true ||
                                ((minIV === 0 ||
                                    (mapData.pokemons[key]["iv"] !== null &&
                                        mapData.pokemons[key]["iv"] >=
                                            minIV)) &&
                                    (minLevel === 0 ||
                                        (mapData.pokemons[key]["level"] !==
                                            null &&
                                            mapData.pokemons[key]["level"] >=
                                                minLevel)));
                            keepMons =
        ...
        if (
            ...
            ((excludedPokemon.indexOf(mapData.pokemons[key]["pokemon_id"]) >=
                0 ||
                isTemporaryHidden(mapData.pokemons[key]["pokemon_id"]) ||
                keepMons === false ||
                (Store.get("showBigKarp") === true &&
                    mapData.pokemons[key]["pokemon_id"] === 129 &&
                    (mapData.pokemons[key]["weight"] < 13.14 ||
                        mapData.pokemons[key]["weight"] === null)) ||
                (Store.get("showTinyRat") === true &&
                    mapData.pokemons[key]["pokemon_id"] === 19 &&
                    (mapData.pokemons[key]["weight"] > 2.4 ||
                        mapData.pokemons[key]["weight"] === null)) ||
                (Store.get("showDespawnTimeType") === 1 &&
                    mapData.pokemons[key]["expire_timestamp_verified"] === 0) ||
                (Store.get("showDespawnTimeType") === 2 &&
                    (mapData.pokemons[key]["expire_timestamp_verified"] > 0 ||
                        mapData.pokemons[key]["spawn_id"] === null)) ||
                (Store.get("showDespawnTimeType") === 3 &&
                    mapData.pokemons[key]["expire_timestamp_verified"] > 0) ||
                (Store.get("showPokemonGender") === 1 &&
                    mapData.pokemons[key]["gender"] !== 1) ||
                (Store.get("showPokemonGender") === 2 &&
                    mapData.pokemons[key]["gender"] !== 2)) &&
    ...
    if (Store.get("showQuests")) {
        ...
        $.each(mapData.pokestops, function (key, value) {
            if (
                Number(mapData.pokestops[key]["quest_timestamp"]) < lastMidnight
            ) {
                if (mapData.pokestops[key].marker.rangeCircle) {
                    markers.removeLayer(
                        mapData.pokestops[key].marker.rangeCircle
                    );
                    delete mapData.pokestops[key].marker.rangeCircle;
                }
                if (mapData.pokestops[key].marker.placementRangeCircle) {
                    markers.removeLayer(
                        mapData.pokestops[key].marker.placementRangeCircle
                    );
    ...
    if (!Store.get("showGyms") && Store.get("showRaids")) {
        $.each(mapData.gyms, function (key, value) {
            if (
                (excludedRaidboss.indexOf(
                    Number(mapData.gyms[key]["raid_pokemon_id"])
                ) > -1 &&
                    mapData.gyms[key]["raid_pokemon_id"] > 0 &&
                    mapData.gyms[key]["raid_start"] < new Date().getTime() &&
                    mapData.gyms[key]["raid_end"] > new Date().getTime()) ||
                (excludedRaidegg.indexOf(
                    Number(mapData.gyms[key]["raid_level"])
                ) > -1 &&
                    mapData.gyms[key]["raid_start"] > new Date().getTime()) ||
                (excludedRaidegg.indexOf(
                    Number(mapData.gyms[key]["raid_level"]) + 15
                ) > -1 &&
                    mapData.gyms[key]["raid_start"] < new Date().getTime() &&
                    mapData.gyms[key]["raid_pokemon_id"] <= 0)
            ) {
                ...
                if (mapData.gyms[key].marker.placementRangeCircle) {
                    markers.removeLayer(
                        mapData.gyms[key].marker.placementRangeCircle
                    );
    ...
    if (Store.get("showNests")) {
        $.each(mapData.nests, function (key, value) {
            if (
                Number(mapData.nests[key]["pokemon_avg"]) <
                Store.get("showNestAvg")
            ) {
        ...
        if (show && placementRangeMarkers.indexOf(type) !== -1) {
            if (!marker.placementRangeCircle) {
                ...
                if (isPlacementRangeActive(map)) {
                    marker.placementRangeCircle = addPlacementRangeCircle(
                        marker,
                        map
                    );
                }
...
function loadRawData() {
    ...
    var independantPvpAndStats = Boolean(
        Store.get("showIndependantPvpAndStats")
    );
    ...
    var loadOverviewStats =
        $("#nav-overview-stats-tab").hasClass("active") && statsOpen;
    var loadPokemonStats =
        $("#nav-pokemon-stats-tab").hasClass("active") && statsOpen;
    var loadRewardStats =
        $("#nav-reward-stats-tab").hasClass("active") && statsOpen;
    var loadShinyStats =
        $("#nav-shiny-stats-tab").hasClass("active") && statsOpen;
    ...
    return $.ajax({
        ...
        beforeSend: function beforeSend() {
            if (
                maxLatLng > 0 &&
                (neLat - swLat > maxLatLng || neLng - swLng > maxLatLng)
            ) {
                sendToast(
                    "warning",
                    i8ln("Max zoom"),
                    i8ln("Please zoom in to get data"),
                    "true"
                );
                return false;
            }
            ...
            switch (xhr.status) {
                case 400:
                    sendToast(
                        ...
                        i8ln("Not Acceptable"),
                        i8ln(
                            "Please check connectivity or reduce marker settings."
                        ),
                        "true"
                    );
                    setTimeout(function () {
                ...
                case 403:
                    sendToast(
                        "danger",
                        i8ln("Forbidden"),
                        i8ln("This action is not allowed."),
                        "true"
                    );
                    setTimeout(function () {
                        window.location.href = "./logout";
                ...
                case 404:
                    sendToast(
                        "danger",
                        i8ln("Not found"),
                        i8ln("Session tokens haven't been found."),
                        "true"
                    );
                    setTimeout(function () {
                        window.location.href =
                            "./login?action=login&error=no-id";
                    }, 5000);
                    break;
                case 413:
                    sendToast(
                        "danger",
                        i8ln("You got me overwhelmed"),
                        i8ln("This is too much data for me please zoom in."),
                        "true"
                    );
                    break;
                default:
                    sendToast(
                        "danger",
                        i8ln("Webserver error"),
                        i8ln("Server went away..."),
                        "true"
                    );
            }
        },
...
function loadWeather() {
    return $.ajax({
        ...
        error: function error() {
            ...
            sendToast(
                "danger",
                i8ln("Error getting weather"),
                i8ln("Please check connectivity or reduce marker settings."),
                "true"
            );
        },
        complete: function complete() {},
                ...
                if (data) {
                    ...
                    $.each(data.reward, function (i, element) {
                        ...
                        $.each(scanAreas, function (index, poly) {
                            var insideScan = turf.booleanPointInPolygon(
                                latlng,
                                poly
                            );
                            if (insideScan) {
                        ...
                        var html =
                            ...
                            '" data-lon="' +
                            element.lon +
                            '"><div class="left-column" onClick="centerMapOnCoords(event);">';
                            ...
                            if (element.quest_reward_type === 7) {
                                html +=
                                    ...
                                    getIcon(
                                        ...
                                        "pokemon",
                                        ".png",
                                        element.reward_pokemon_id,
                                        0,
                                        element.reward_pokemon_formid,
                                        element.reward_pokemon_costumeid,
                                        element.reward_pokemon_genderid,
                                        element.reward_pokemon_shiny
                                    ) +
                                    ') no-repeat;" class="i-icon" ></span>';
                            }
                            if (element.quest_reward_type === 2) {
                                html +=
                                    ...
                                    getIcon(
                                        ...
                                        "reward/item",
                                        ".png",
                                        element.reward_item_id,
                                        element.reward_amount
                                    ) +
                                    ') no-repeat;" class="i-icon" ></span>';
                            }
                        ...
                        if (sr.hasClass("reward-results")) {
                            if (element.reward_pokemon_name !== null) {
                                html +=
                                    '<span class="reward" style="font-weight:bold">' +
                                    element.reward_pokemon_name +
                                    "</span><span>&nbsp;-&#32;</span>";
                            }
                            if (element.reward_item_name !== null) {
                                html +=
                                    '<span class="reward" style="font-weight:bold">' +
                                    element.reward_item_name +
                                    "</span><span>&nbsp;-&#32;</span>";
                            }
                        }
                        html +=
                            ...
                            element.name +
                            "</span>" +
                            '<span class="distance" style="font-weight:bold">&nbsp;-&#32;' +
                            element.distance +
                            defaultUnit +
                            "</span>";
                        html += "</div></div>";
                        if (
                            sr.hasClass("pokestop-results") &&
                            !noManualQuests &&
                            !scanArea
                        ) {
                            html +=
                                '<div class="right-column"><i class="fas fa-binoculars submit-quests"  onClick="openQuestModal(event);" data-id="' +
                                element.external_id +
                                '"></i></div>';
                        } else {
                            html +=
                                '<div class="right-column" onClick="centerMapOnCoords(event);"><span style="background:url(' +
                                element.url +
                                ') no-repeat;" class="i-icon" ></span></div>';
                        }
                        html += "</li>";
                        sr.append(html);
                    });
                    $.each(data.forts, function (i, element) {
                        ...
                        html +=
                            ...
                            "</div></div>";
                        if (
                            sr.hasClass("gym-results") &&
                            manualRaids &&
                            !scanArea
                        ) {
                            html +=
                                '<div class="right-column"><i class="fas fa-binoculars submit-raid"  onClick="openRaidModal(event);" data-id="' +
                                element.external_id +
                                '"></i></div>';
                        }
                        html += "</li>";
...
function manualPokestopData(event) {
    // eslint-disable-line no-unused-vars
    var form = $(event.target).parent().parent();
    ...
    if (pokestopName && pokestopName !== "" && !scanArea) {
        if (
            confirm(
                i8ln(
                    "I confirm this is an accurate reporting of a new pokestop"
                )
            )
        ) {
            return $.ajax({
                ...
                error: function error() {
                    ...
                    sendToast(
                        "danger",
                        i8ln("Error Submitting Pokéstop"),
                        i8ln("Could not submit Pokéstop."),
                        "true"
                    );
                },
                complete: function complete() {
                    sendToast(
                        "success",
                        i8ln("Submitting Pokéstop"),
                        pokestopName + " " + i8ln("successful submitted."),
                        "true"
                    );
                    lastpokestops = false;
                    updateMap();
        ...
        if (
            confirm(
                i8ln("Adding a Pokéstop inside the scan area is not allowed")
            )
        ) {
            $(".modal").modal("hide");
        }
...
function manualGymData(event) {
    ...
    if (gymName && gymName !== "" && !scanArea) {
        if (
            confirm(
                i8ln("I confirm this is an accurate reporting of a new gym")
            )
        ) {
            return $.ajax({
                url: "submit",
                ...
                error: function error() {
                    ...
                    sendToast(
                        "danger",
                        i8ln("Error Submitting Gym"),
                        i8ln("Could not submit Gym."),
                        "true"
                    );
                },
                complete: function complete() {
                    sendToast(
                        "danger",
                        i8ln("Submitting Gym"),
                        gymName + " " + i8ln("successful submitted."),
                        "true"
                    );
                    lastgyms = false;
                    updateMap();
...
function manualPokemonData(event) {
    // eslint-disable-line no-unused-vars
    ...
    if (pokemonId && pokemonId !== "" && !scanArea) {
        if (
            confirm(
                i8ln("I confirm this is an accurate reporting of a new pokemon")
            )
        ) {
            return $.ajax({
                url: "submit",
                ...
                error: function error() {
                    ...
                    sendToast(
                        "danger",
                        i8ln("Error Submitting Pokémon"),
                        i8ln("Could not submit Pokémon."),
                        "true"
                    );
                },
                complete: function complete() {
                    sendToast(
                        "success",
                        i8ln("Submitting Pokémon"),
                        pokeList[pokemonId - 1].name +
                            " " +
                            i8ln("successful submitted."),
                        "true"
                    );
                    lastpokemon = false;
                    updateMap();
        ...
        if (
            confirm(
                i8ln("Adding a wild spawn inside the scan area is not allowed")
            )
        ) {
            $(".modal").modal("hide");
        }
...
function deleteGym(event) {
    ...
    if (gymId && gymId !== "") {
        if (
            confirm(
                i8ln(
                    "I confirm that I want to delete this gym. This is a permanent removal"
                )
            )
        ) {
            return $.ajax({
                ...
                error: function error() {
                    ...
                    sendToast(
                        "danger",
                        i8ln("Error Deleting Gym"),
                        i8ln("Could not delete Gym."),
                        "true"
                    );
                },
                complete: function complete() {
...
function renameGymData(event) {
    ...
    if (gymName && gymName !== "") {
        if (
            confirm(i8ln("I confirm this is an accurate new name for this gym"))
        ) {
            return $.ajax({
                url: "submit",
                type: "POST",
            ...
            return $.ajax({
                ...
                error: function error() {
                    ...
                    sendToast(
                        "danger",
                        i8ln("Error EX Gym"),
                        i8ln("Could not change Gym to EX Gym."),
                        "true"
                    );
                },
                complete: function complete() {
                    sendToast(
                        "success",
                        i8ln("Change EX Gym"),
                        i8ln("Successful changed Gym to EX Gym."),
                        "true"
                    );
                    jQuery('label[for="gyms-switch"]').click();
                    jQuery('label[for="gyms-switch"]').click();
...
function deletePokestop(event) {
    ...
    if (pokestopId && pokestopId !== "") {
        if (
            confirm(
                i8ln(
                    "I confirm that I want to delete this pokestop. This is a permanent removal"
                )
            )
        ) {
            return $.ajax({
                ...
                error: function error() {
                    ...
                    sendToast(
                        "danger",
                        i8ln("Deleting Pokéstop"),
                        i8ln("Could not delete Pokéstop."),
                        "true"
                    );
                },
                complete: function complete() {
                    sendToast(
                        "success",
                        i8ln("Deleting Pokéstop"),
                        i8ln("Successful deleted Pokéstop."),
                        "true"
                    );
...
function renamePokestopData(event) {
    // eslint-disable-line no-unused-vars
    var form = $(event.target).parent().parent();
    ...
    if (pokestopName && pokestopName !== "") {
        if (
            confirm(
                i8ln("I confirm this is an accurate new name for this pokestop")
            )
        ) {
            return $.ajax({
                url: "submit",
                ...
                error: function error() {
                    ...
                    sendToast(
                        "danger",
                        i8ln("Rename Pokéstop"),
                        i8ln("Could not rename Pokéstop."),
                        "true"
                    );
                },
                complete: function complete() {
...
function convertPokestopData(event) {
    // eslint-disable-line no-unused-vars
    var form = $(event.target).parent().parent();
        ...
        if (confirm(i8ln("I confirm this pokestop is now a gym"))) {
            return $.ajax({
                ...
                error: function error() {
                    ...
                    sendToast(
                        "danger",
                        i8ln("Error converting Pokéstop"),
                        i8ln("Pokéstop ID got lost somewhere."),
                        "true"
                    );
                },
                complete: function complete() {
                    sendToast(
                        "success",
                        i8ln("Converting Pokéstop"),
                        i8ln("Pokéstop converted to Gym"),
                        "true"
                    );
                    lastgyms = false;
                    jQuery('label[for="pokestops-switch"]').click();
...
function convertPortalData(event, targetType) {
    // eslint-disable-line no-unused-vars
    var form = $(event.target).parent().parent();
            ...
            return $.ajax({
                ...
                error: function error() {
                    ...
                    sendToast(
                        "danger",
                        errorText,
                        i8ln("Portal ID got lost somewhere."),
                        "true"
                    );
                },
                complete: function complete() {
                    switch (targetType) {
                        case "1":
                            sendToast(
                                "success",
                                i8ln("Convert Portal"),
                                i8ln("Portal successful converted to Pokéstop"),
                                "true"
                            );
                            jQuery('label[for="pokestops-switch"]').click();
                            jQuery('label[for="pokestops-switch"]').click();
                        ...
                        case "2":
                            sendToast(
                                "success",
                                i8ln("Convert Portal"),
                                i8ln("Portal successful converted to Gym"),
                                "true"
                            );
                            lastgyms = false;
                            break;
            ...
            return $.ajax({
                ...
                complete: function complete() {
                    sendToast(
                        "success",
                        i8ln("Marking portal"),
                        i8ln("Portal marked as not a Pokéstop or Gym"),
                        "true"
                    );
                    jQuery('label[for="portals-switch"]').click();
                    jQuery('label[for="portals-switch"]').click();
...
function deleteNest(event) {
    ...
    if (nestid && nestid !== "") {
        if (
            confirm(
                i8ln(
                    "I confirm that I want to delete this nest. This is a permanent removal"
                )
            )
        ) {
            return $.ajax({
                ...
                error: function error() {
                    ...
                    sendToast(
                        "danger",
                        i8ln("Error Deleting nest"),
                        i8ln("Could not delete Nest"),
                        "true"
                    );
                },
                complete: function complete() {
        ...
        if (confirm(i8ln("I confirm this is an new nest"))) {
            return $.ajax({
                ...
                error: function error() {
                    ...
                    sendToast(
                        "danger",
                        i8ln("Submitting nest"),
                        i8ln("Could not submit Nest"),
                        "true"
                    );
                },
                complete: function complete() {
                    sendToast(
                        ...
                        pokeList[pokemonId - 1].name +
                            " " +
                            i8ln("nest successful submitted"),
                        "true"
                    );
                    lastnests = false;
                    updateMap();
...
function manualNestData(event) {
    ...
    if (nestId && nestId !== "" && pokemonId && pokemonId !== "") {
        if (
            confirm(i8ln("I confirm this is an accurate sighting of a quest"))
        ) {
            return $.ajax({
                url: "submit",
                type: "POST",
                ...
                complete: function complete() {
                    sendToast(
                        "success",
                        i8ln("Submitting nest"),
                        i8ln("Nest changed to ") + pokeList[pokemonId - 1].name,
                        "true"
                    );
                    lastnests = false;
                    updateMap();
...
function manualQuestData(event) {
    // eslint-disable-line no-unused-vars
        ...
        if (
            ...
            return $.ajax({
                ...
                error: function error() {
                    ...
                    sendToast(
                        "danger",
                        i8ln("Submitting quest"),
                        i8ln("Could not submit quest"),
                        "true"
                    );
                },
                complete: function complete() {
                    sendToast(
                        "success",
                        i8ln("Submitting quest"),
                        i8ln("Successful submitted quest"),
                        "true"
                    );
                    lastpokestops = false;
                    updateMap();
        ...
        if (confirm(i8ln("I confirm this is an accurate sighting of a raid"))) {
            return $.ajax({
                ...
                error: function error() {
                    ...
                    sendToast(
                        "danger",
                        i8ln("Submitting raid"),
                        i8ln("Could not submit raid"),
                        "true"
                    );
                },
                complete: function complete() {
...
function submitNewCommunity(event) {
    // eslint-disable-line no-unused-vars
    var form = $(event.target).parent().parent();
    var lat = $("#submitModal .submitLatitude").val();
    var lon = $("#submitModal .submitLongitude").val();
    var communityName = form.find('[name="community-name"]').val();
    var communityDescription = form
        .find('[name="community-description"]')
        .val();
        ...
        if (confirm(i8ln("I confirm this is an active community"))) {
            return $.ajax({
                ...
                error: function error() {
                    ...
                    sendToast(
                        "danger",
                        i8ln("Submitting community"),
                        i8ln(
                            "Make sure all fields are filled and the invite link is a valid Discord, Telegram or Whatsapp link."
                        ),
                        "true"
                    );
                },
                complete: function complete() {
                    sendToast(
                        "success",
                        i8ln("Submitting community"),
                        communityName + " " + i8ln("community submitted"),
                        "true"
                    );
...
function editCommunityData(event) {
    // eslint-disable-line no-unused-vars
    var form = $(event.target).parent().parent();
    ...
    if (
        ...
        if (
            confirm(i8ln("I confirm this new info accurate for this community"))
        ) {
            return $.ajax({
                url: "submit",
                type: "POST",
                ...
                error: function error() {
                    ...
                    sendToast(
                        ...
                        i8ln("Edit community"),
                        i8ln(
                            "Form fields are empty or an invalid url is found, please check the form."
                        ),
                        "true"
                    );
                },
                complete: function complete() {
                    sendToast(
                        "success",
                        i8ln("Edit community"),
                        communityName + " " + i8ln("edit successful"),
                        "true"
                    );
                    jQuery('label[for="communities-switch"]').click();
                    jQuery('label[for="communities-switch"]').click();
...
function deleteCommunity(event) {
    ...
    if (communityid && communityid !== "") {
        if (
            confirm(
                i8ln(
                    "I confirm that I want to delete this community. This is a permanent removal"
                )
            )
        ) {
            return $.ajax({
                ...
                complete: function complete() {
                    sendToast(
                        "success",
                        i8ln("Delete community"),
                        i8ln("Community successful deleted"),
                        "true"
                    );
                    jQuery('label[for="communities-switch"]').click();
                    jQuery('label[for="communities-switch"]').click();
...
function submitPoi(event) {
    ...
    var poiSurrounding = form
        .find('[name="preview-poi-surrounding"]')
        .attr("src");
            ...
            return $.ajax({
                ...
                complete: function complete() {
                    sendToast(
                        "success",
                        i8ln("Submit POI"),
                        poiName + " " + i8ln("successful submitted"),
                        "true"
                    );
                    jQuery('label[for="poi-switch"]').click();
                    jQuery('label[for="poi-switch"]').click();
...
function editPoiData(event) {
    ...
    var poiSurrounding = form
        .find('[id="edit-preview-poi-surrounding"]')
        .attr("src");
    if (typeof poiImage !== "undefined") {
        poiImage = poiImage.split(",")[1];
    } else {
            ...
            return $.ajax({
                ...
                complete: function complete() {
                    sendToast(
                        "success",
                        i8ln("Edit POI"),
                        poiName + " " + i8ln("successful editted"),
                        "true"
                    );
                    jQuery('label[for="poi-switch"]').click();
                    jQuery('label[for="poi-switch"]').click();
...
function markPoi(event, poiMarkType) {
    // eslint-disable-line no-unused-vars
    ...
    if (poiId && poiId !== "") {
        switch (poiMarkType) {
            case "2":
                confirmText = i8ln(
                    "I confirm this candidate is submitted to OPR"
                );
                break;
            case "3":
                confirmText = i8ln(
                    "I confirm this candidate is declined by OPR"
                );
                break;
            case "4":
                confirmText = i8ln(
                    "I confirm this candidate is declined by OPR but can be resubmitted as candidate"
                );
                break;
            case "5":
                confirmText = i8ln(
                    "I confirm this is not a eligible candidate to submit to OPR"
                );
                break;
        }
        if (confirm(confirmText)) {
...
function openRenamePokestopModal(event) {
    // eslint-disable-line no-unused-vars
    $(".modal").modal("hide");
    var val = $(event.target).data("id");
...
function openRenameGymModal(event) {
    // eslint-disable-line no-unused-vars
    $(".modal").modal("hide");
    var val = $(event.target).data("id");
    $("#renamegymid").val(val);
...
function openConvertPokestopModal(event) {
    // eslint-disable-line no-unused-vars
    $(".modal").modal("hide");
    var val = $(event.target).data("id");
...
function openConvertPortalModal(event) {
    // eslint-disable-line no-unused-vars
    $(".modal").modal("hide");
    var val = $(event.target).data("id");
...
function openEditCommunityModal(event) {
    // eslint-disable-line no-unused-vars
    $(".modal").modal("hide");
    var val = $(event.target).data("id");
...
function openEditPoiModal(event) {
    // eslint-disable-line no-unused-vars
    $(".modal").modal("hide");
    var val = $(event.target).data("id");
...
function openMarkPoiModal(event) {
    // eslint-disable-line no-unused-vars
    $(".modal").modal("hide");
    var val = $(event.target).data("id");
    $("#markpoiid").val(val);
...
function processPokemons(i, item) {
    ...
    if (
        ...
        ((encounterId && encounterId === item["encounter_id"]) ||
            (excludedPokemon.indexOf(item["pokemon_id"]) < 0 &&
                !isTemporaryHidden(item["pokemon_id"])))
    ) {
        if (item["encounter_id"] in mapData.pokemons) {
            if (
                mapData.pokemons[item["encounter_id"]]["spawn_id"] !==
                    item["spawn_id"] ||
                mapData.pokemons[item["encounter_id"]]["pokemon_id"] !==
                    item["pokemon_id"] ||
                mapData.pokemons[item["encounter_id"]]["individual_attack"] !==
                    item["individual_attack"] ||
                mapData.pokemons[item["encounter_id"]]["individual_defense"] !==
                    item["individual_defense"] ||
                mapData.pokemons[item["encounter_id"]]["individual_stamina"] !==
                    item["individual_stamina"]
            ) {
                // updated information received. delete marker and item from dict
                if (mapData.pokemons[item["encounter_id"]].marker.rangeCircle) {
                    markers.removeLayer(
                        mapData.pokemons[item["encounter_id"]].marker
                            .rangeCircle
                    );
                    delete mapData.pokemons[item["encounter_id"]].marker
                        .rangeCircle;
                }
                markers.removeLayer(
                    mapData.pokemons[item["encounter_id"]].marker
...
function processPokestops(i, item, lastMidnight) {
    ...
    var removePokestopFromMap = function removePokestopFromMap(pokestopid) {
        if (
            mapData.pokestops[pokestopid] &&
            mapData.pokestops[pokestopid].marker
        ) {
            if (mapData.pokestops[pokestopid].marker.rangeCircle) {
                markers.removeLayer(
                    mapData.pokestops[pokestopid].marker.rangeCircle
                );
            }
            if (mapData.pokestops[pokestopid].marker.placementRangeCircle) {
                markers.removeLayer(
    ...
    if (
        Store.get("showQuests") &&
        !pokestopMeetsQuestFilter(item, lastMidnight)
    ) {
        removePokestopFromMap(item["pokestop_id"]);
        return true;
    }
    ...
    if (item["pokestop_id"] in mapData.pokestops) {
        ...
        item.marker = updatePokestopMarker(
            item,
            mapData.pokestops[item["pokestop_id"]].marker
        );
    } else {
...
function pokestopMeetsQuestFilter(pokestop, lastMidnight) {
    if (
        ...
        lastMidnight > Number(pokestop["quest_timestamp"])
    ) {
        return false;
    } else if (
        pokestop["quest_reward_type"] === 12 &&
        pokestop["reward_pokemon_id"] > 0 &&
        questsExcludedEnergy.indexOf(pokestop["reward_pokemon_id"]) > -1
    ) {
        return false;
    } else if (
        pokestop["quest_reward_type"] === 4 &&
        pokestop["reward_pokemon_id"] > 0 &&
        questsExcludedCandy.indexOf(pokestop["reward_pokemon_id"]) > -1
    ) {
        return false;
    } else if (
        pokestop["quest_reward_type"] === 7 &&
        pokestop["reward_pokemon_id"] > 0 &&
        questsExcludedPokemon.indexOf(pokestop["reward_pokemon_id"]) > -1
    ) {
        return false;
    } else if (
        pokestop["quest_reward_type"] === 2 &&
        pokestop["reward_item_id"] > 0 &&
        questsExcludedItem.indexOf(pokestop["reward_item_id"]) > -1
    ) {
        return false;
    } else if (
        pokestop["quest_reward_type"] === 3 &&
        (Store.get("showDustAmount") === 0 ||
            Number(pokestop["reward_amount"]) <
                Number(Store.get("showDustAmount")))
    ) {
        return false;
    } else if (
        pokestop["quest_reward_type"] === 1 &&
        (Store.get("showXpAmount") === 0 ||
            Number(pokestop["reward_amount"]) <
                Number(Store.get("showXpAmount")))
    ) {
        return false;
    } else {
        return true;
...
function updatePokestops() {
    ...
    $.each(mapData.pokestops, function (key, value) {
        ...
        if (
            value["lure_expiration"] > 0 &&
            value["lure_expiration"] < currentTime &&
            value["lure_expiration"] > currentTime - 300000
        ) {
            value.marker = updatePokestopMarker(value, value.marker);
        }
        // change Team Rocket pokestop marker to normal when expired.
        if (
            value["incident_expiration"] > 0 &&
            value["incident_expiration"] < currentTime &&
            value["incident_expiration"] > currentTime - 300000
        ) {
            value.marker = updatePokestopMarker(value, value.marker);
        }
    });
        ...
        $.each(removeStops, function (key, value) {
            if (mapData.pokestops[value] && mapData.pokestops[value].marker) {
                if (mapData.pokestops[value].marker.rangeCircle) {
                    markers.removeLayer(
                        mapData.pokestops[value].marker.rangeCircle
                    );
                }
                if (mapData.pokestops[value].marker.placementRangeCircle) {
                    markers.removeLayer(
                        mapData.pokestops[value].marker.placementRangeCircle
                    );
                }
    ...
    if (Store.get("showRocket")) {
        $.each(mapData.pokestops, function (key, value) {
            if (
                value["incident_expiration"] < currentTime ||
                excludedGrunts.indexOf(Number(value["grunt_type"])) > -1
            ) {
                removeStops.push(key);
            }
        });
...
function processGyms(i, item) {
    ...
    var removeGymFromMap = function removeGymFromMap(gymid) {
        if (mapData.gyms[gymid] && mapData.gyms[gymid].marker) {
            ...
            if (mapData.gyms[gymid].marker.placementRangeCircle) {
                markers.removeLayer(
                    mapData.gyms[gymid].marker.placementRangeCircle
                );
            }
            markers.removeLayer(mapData.gyms[gymid].marker);
    ...
    if (Store.get("showGyms") && !Store.get("showRaids")) {
        item.raid_end = 0;
        item.raid_level =
            item.raid_pokemon_cp =
            item.raid_pokemon_id =
            item.raid_pokemon_move_1 =
            item.raid_pokemon_move_1 =
            item.raid_pokemon_name =
                null;
    }
    ...
    if (Store.get("showGyms") && Store.get("showRaids")) {
        ...
        if (
            !noActiveRaids &&
            Store.get("activeRaids") &&
            item.raid_end > Date.now()
        ) {
            if (
                (item.raid_pokemon_id === undefined ||
                    item.raid_pokemon_id === null) &&
                item.raid_start > Date.now()
            ) {
                item.raid_end = 0;
                item.raid_level =
        ...
        if (!noMinMaxRaidLevel) {
            if (
                (raidLevel < Store.get("minRaidLevel") &&
                    item.raid_end > Date.now()) ||
                (raidLevel > Store.get("maxRaidLevel") &&
                    item.raid_end > Date.now())
            ) {
                item.raid_end = 0;
                item.raid_level =
    ...
    if (!noOpenSpot && Store.get("showOpenGymsOnly")) {
        if (
            ...
            (item.raid_end === undefined || item.raid_end < Date.now())
        ) {
            removeGymFromMap(item["gym_id"]);
            return true;
        }
    ...
    if (
        !noTeams &&
        Store.get("showTeamGymsOnly") &&
        Store.get("showTeamGymsOnly") !== item.team_id &&
        (item.raid_end === undefined || item.raid_end < Date.now())
    ) {
        removeGymFromMap(item["gym_id"]);
        return true;
    ...
    if (!noLastScan && Store.get("showLastUpdatedGymsOnly")) {
        ...
        if (item.last_scanned == null) {
            if (
                Store.get("showLastUpdatedGymsOnly") * 3600 * 1000 +
                    item.last_modified <
                    now.getTime() &&
                (item.raid_end === undefined || item.raid_end < Date.now())
            ) {
    ...
    if (!noMinMaxFreeSlots) {
        if (
            gymLevel < Store.get("minGymLevel") &&
            (item.raid_end === undefined || item.raid_end < Date.now())
        ) {
            removeGymFromMap(item["gym_id"]);
            return true;
    ...
    if (item["gym_id"] in mapData.gyms) {
        item.marker = updateGymMarker(
            item,
            mapData.gyms[item["gym_id"]].marker
        );
    } else {
        // add marker to map and item to dict
...
function processScanlocation(i, item) {
    ...
    if (
        oldLoc !== null &&
        (oldLoc[0] !== newLoc[0] || oldLoc[1] !== newLoc[1])
    ) {
...
function updateMap() {
    if (_mapLoaded) {
        ...
        var s2CellCenter = S2.keyToId(
            S2.latLngToKey(position.lat, position.lng, 10)
        );
        if (
            s2CellCenter &&
            (String(s2CellCenter) !==
                $("#currentWeather").data("current-cell") ||
                $("#currentWeather").data("updated") < currentHourTime) &&
            map.getZoom() > 13
        ) {
            loadWeatherCellData(s2CellCenter).done(function (cellWeather) {
                ...
                if (currentWeather) {
                    ...
                    var boostedTypesText =
                        weatherBoostedTypes[currentWeather.condition];
                    var markerTitle =
                        "Weather: " +
                        weatherText +
                        (currentWeather.updated >= currentHourTime
                            ? ""
                            : " (Out of date)") +
                        "\nBoosted Types: " +
                        boostedTypesText +
                        "\nLast Updated: " +
                        moment(currentWeather.updated * 1000).format(
                            "dddd, Do MMMM Y, HH:mm"
                        );

                    $("#currentWeather").data(
                        "current-cell",
                        currentWeather.s2_cell_id
                    );
                    $("#currentWeather").data(
                        "updated",
    ...
    if (weather) {
        $.each(weather, function (idx, item) {
            ...
            var markerTitle =
                "Weather: " +
                weatherText +
                (weatherTime >= currentHourTime ? "" : " (Out of date)") +
                "\nBoosted Types: " +
                boostedTypesText +
                "\nLast Updated: " +
                moment(weatherTime).format("dddd, Do MMMM Y, HH:mm");
            if (map.getZoom() <= 13) {
                $.each(weatherMarkers, function (index, marker) {
                    markersnotify.addLayer(marker);
                });
            ...
            var icon = L.icon({
                ...
                iconUrl:
                    "static/weather/a-" +
                    item.condition +
                    (weatherTime >= currentHourTime ? ".png" : "-ood.png"),
            });
...
function redrawPokemon(pokemonList) {
    ...
    $.each(pokemonList, function (key, value) {
        ...
        if (!item.hidden) {
            if (item.marker.rangeCircle)
                markers.removeLayer(item.marker.rangeCircle);
            var newMarker = setupPokemonMarker(
                item,
                map,
...
var updateLabelDiffTime = function updateLabelDiffTime() {
    $(".label-countdown").each(function (index, element) {
        var disappearsAt = getTimeUntil(
            parseInt(element.getAttribute("disappears-at"))
        );
...
function sendToast(alertClass, headerText, bodyText, autoHide) {
    ...
    var toastStr =
        ...
        identifier +
        '" class="toast fade" role="alert" data-bs-autohide="' +
        autoHide +
        '" data-toast-timestamp="' +
        timestamp +
...
function sendToastPokemon(title, text, icon, lat, lon) {
    ...
    var toastStr =
        ...
        identifier +
        '" class="toast fade" role="alert" data-toast-timestamp="' +
        timestamp +
        '" aria-live="assertive" aria-atomic="true">' +
        '<div class="toast-header ' +
        identifier +
        ...
        text +
        "</div>" +
        '<div class="col-2" onclick="centerMap(' +
        lat +
        "," +
        lon +
        ', 20)">' +
...
function cleanOldToasts() {
    $("[id^=toast-message-]").each(function (index, element) {
        if (
            $(element).data("toast-timestamp") <
            timestamp - toastDelay / 1000
        ) {
            $(element).remove();
        }
    });
    $("[id^=toast-pokemon-]").each(function (index, element) {
        if (
            $(element).data("toast-timestamp") <
            timestamp - Store.get("toastPokemonDelay") / 1000
        ) {
            $(element).remove();
        }
    });
...
function createMyLocationButton() {
    ...
    _locationMarker.onAdd = function (map) {
        ...
        locationIcon.style.backgroundImage =
            "url(static/mylocation-sprite-1x.png)";
...
function i8ln(word) {
    if (
        $.isEmptyObject(i8lnDictionary) &&
        language !== "en" &&
        languageLookups < languageLookupThreshold
    ) {
        $.ajax({
                ...
                if (typeof locationMarker !== "undefined") {
                    ...
                    if (Store.get("spawnArea")) {
                        if (locationMarker.rangeCircle) {
                            ...
                            locationMarker.rangeCircle = L.circle(
                                center,
                                rangeCircleOpts
                            );
    ...
    try {
        if (isMobileDevice() && window.Worker) {
            ...
            updateWorker.onmessage = function (e) {
                ...
                if (
                    document.hidden &&
                    data.name === "backgroundUpdate" &&
                    Date.now() - lastUpdateTime > 2500
                ) {
                    updateMap();
                    updateGeoLocation();
                }
                if (
                    document.hidden &&
                    data.name === "backgroundUpdate" &&
                    Date.now() - lastWeatherUpdateTime > 60000
                ) {
                    updateWeatherOverlay();
                }
            };
...
function pokemonSubmitFilter(event) {
    // eslint-disable-line no-unused-vars
...
function pokemonSpritesFilter() {
    jQuery(".offcanvas-body.left .pokemon-list .pokemon-icon-sprite").on(
        "click",
        function () {
            var img = jQuery(this);
...
function energySpritesFilter() {
    jQuery(".offcanvas-body.left .energy-list .energy-icon-sprite").on(
        "click",
        function () {
            var img = jQuery(this);
            var select = jQuery(this)
...
function candySpritesFilter() {
    jQuery(".offcanvas-body.left .candy-list .candy-icon-sprite").on(
        "click",
        function () {
            var img = jQuery(this);
            var select = jQuery(this)
...
function itemSpritesFilter() {
    jQuery(".offcanvas-body.left .item-list .item-icon-sprite").on(
        "click",
        function () {
            var img = jQuery(this);
            var select = jQuery(this)
...
function gruntSpritesFilter() {
    jQuery(".offcanvas-body.left .grunt-list .grunt-icon-sprite").on(
        "click",
        function () {
            var img = jQuery(this);
            var select = jQuery(this)
...
function raideggSpritesFilter() {
    jQuery(".offcanvas-body.left .raidegg-list .raidegg-icon-sprite").on(
        "click",
        function () {
            var img = jQuery(this);
            var select = jQuery(this)
...
$(function () {
    ...
    // load MOTD, if set
    if (
        (motd && !showMotdOnlyOnce) ||
        (motd && showMotdOnlyOnce && Store.get("oldMotd") !== motdContent)
    ) {
...
$(function () {
    ...
    $selectDirectionProvider
        .val(Store.get("directionProvider"))
        .trigger("change");
        ...
        $.each(["gyms"], function (d, dType) {
            $.each(mapData[dType], function (key, value) {
                ...
                if (mapData[dType][key].marker.placementRangeCircle) {
                    markers.removeLayer(
                        mapData[dType][key].marker.placementRangeCircle
                    );
    ...
    if (
        Store.get("iconsArray").gym === undefined ||
        Object.prototype.toString.call(Store.get("iconsArray").gym) ===
            "[object Object]" ||
        Store.get("iconsArray").gym === ""
    ) {
        iconFolderArray["gym"] =
            iconFolderArray["gym"][Object.keys(iconFolderArray["gym"])[0]];
        Store.set("iconsArray", iconFolderArray);
    }
    if (
        Store.get("iconsArray").pokemon === undefined ||
        Object.prototype.toString.call(Store.get("iconsArray").pokemon) ===
            "[object Object]" ||
        Store.get("iconsArray").pokemon === ""
    ) {
        iconFolderArray["pokemon"] =
            iconFolderArray["pokemon"][
                Object.keys(iconFolderArray["pokemon"])[0]
            ];
        Store.set("iconsArray", iconFolderArray);
    }
    if (
        Store.get("iconsArray").reward === undefined ||
        Object.prototype.toString.call(Store.get("iconsArray").reward) ===
            "[object Object]" ||
        Store.get("iconsArray").reward === ""
    ) {
        iconFolderArray["reward"] =
            iconFolderArray["reward"][
                Object.keys(iconFolderArray["reward"])[0]
            ];
        Store.set("iconsArray", iconFolderArray);
    }
    ...
    $selectGymMarkerStyle.on("change", function (e) {
        ...
        if (enableJSDebug) {
            console.log(
                "Attempting to load Gym Index: " + newIconSet + "gym/index.json"
            );
        }
        $.ajax({
            cache: false,
    ...
    $selectIconStyle.on("change", function (e) {
        ...
        if (enableJSDebug) {
            console.log(
                "Attempting to load Pokemon Index: " +
                    newIconSet +
                    "pokemon/index.json"
            );
        }
        $.ajax({
            ...
            .done(function (data) {
                if (enableJSDebug) {
                    console.log(
                        "Successfully loaded Pokemon Index: " +
                            newIconSet +
                            "pokemon/index.json"
                    );
                }
    ...
    $selectRewardIconStyle.on("change", function (e) {
        ...
        if (enableJSDebug) {
            console.log(
                "Attempting to load Reward Index: " +
                    newIconSet +
                    "reward/index.json"
            );
        }
        $.ajax({
            ...
            .done(function (data) {
                if (enableJSDebug) {
                    console.log(
                        "Successfully loaded Reward Index: " +
                            newIconSet +
                            "reward/index.json"
                    );
                }
...
$(function () {
    ...
    excludedPokemon = String(Store.get("remember_select_exclude"))
        .split(",")
        .map(Number)
        .sort(function (a, b) {
            return parseInt(a) - parseInt(b);
        });
    excludedMinIV = String(Store.get("remember_select_exclude_min_iv"))
        .split(",")
        .map(Number)
        .sort(function (a, b) {
            return parseInt(a) - parseInt(b);
        });
    excludedRaidboss = String(Store.get("remember_exclude_raidboss"))
        .split(",")
        .map(Number)
        .sort(function (a, b) {
            return parseInt(a) - parseInt(b);
        });
    excludedRaidegg = String(Store.get("remember_exclude_raidegg"))
        .split(",")
        .map(Number)
        .sort(function (a, b) {
            return parseInt(a) - parseInt(b);
        });
    excludedGrunts = String(Store.get("remember_exclude_grunts"))
        .split(",")
        .map(Number)
        .sort(function (a, b) {
            return parseInt(a) - parseInt(b);
        });
    questsExcludedCandy = String(Store.get("remember_quests_exclude_candy"))
        .split(",")
        .map(Number)
        .sort(function (a, b) {
            return parseInt(a) - parseInt(b);
        });
    questsExcludedEnergy = String(Store.get("remember_quests_exclude_energy"))
        .split(",")
        .map(Number)
        .sort(function (a, b) {
            return parseInt(a) - parseInt(b);
        });
    questsExcludedItem = String(Store.get("remember_quests_exclude_item"))
        .split(",")
        .map(Number)
        .sort(function (a, b) {
            return parseInt(a) - parseInt(b);
        });
    questsExcludedPokemon = String(Store.get("remember_quests_exclude_pokemon"))
        .split(",")
        .map(Number)
        .sort(function (a, b) {
            return parseInt(a) - parseInt(b);
        });
    notifiedPokemon = String(Store.get("remember_select_notify"))
        .split(",")
        .map(Number)
        .sort(function (a, b) {
            return parseInt(a) - parseInt(b);
    ...
    $.getJSON("static/dist/data/questtype.min.json", {
        _: new Date().getTime(),
    }).done(function (data) {
    ...
    $.getJSON("static/dist/data/raidegg.min.json").done(function (data) {
        ...
        $excludeRaidegg.on("change", function (e) {
            ...
            excludedRaidegg = $excludeRaidegg
                .val()
                .split(",")
                .map(Number)
                .sort(function (a, b) {
    ...
    $.getJSON("static/dist/data/items.min.json").done(function (data) {
        ...
        $questsExcludeItem.on("change", function (e) {
            ...
            questsExcludedItem = $questsExcludeItem
                .val()
                .split(",")
                .map(Number)
                .sort(function (a, b) {
            ...
            reincludedQuestsItem = reincludedQuestsItem
                .concat(buffer)
                .map(String);
            updateMap();
    ...
    $.getJSON("static/dist/data/pokemon.min.json").done(function (data) {
        ...
        $selectExcludeMinIV.on("change", function (e) {
            ...
            excludedMinIV = $selectExcludeMinIV
                .val()
                .split(",")
                .map(Number)
                .sort(function (a, b) {
        ...
        $textMinLLRank.on("change", function (e) {
            minLLRank = Math.max(
                0,
                Math.min(parseInt($textMinLLRank.val(), 0) || 0, 100)
            );
        ...
        $textMinGLRank.on("change", function (e) {
            minGLRank = Math.max(
                0,
                Math.min(parseInt($textMinGLRank.val(), 0) || 0, 100)
            );
        ...
        $textMinULRank.on("change", function (e) {
            minULRank = Math.max(
                0,
                Math.min(parseInt($textMinULRank.val(), 0) || 0, 100)
            );
        ...
        $textMinIV.on("change", function (e) {
            minIV = Math.max(
                0,
                Math.min(parseInt($textMinIV.val(), 10) || 0, 100)
            );
            $textMinIV.val(minIV);
        ...
        $textMinLevel.on("change", function (e) {
            minLevel = Math.max(
                0,
                Math.min(parseInt($textMinLevel.val(), 10) || 0, 35)
            );
        ...
        $selectPokemonNotify.on("change", function (e) {
            notifiedPokemon = $selectPokemonNotify
                .val()
                .split(",")
                .map(Number)
                .sort(function (a, b) {
        ...
        $textPerfectionNotify.on("change", function (e) {
            notifiedMinPerfection = Math.max(
                0,
                Math.min(parseInt($textPerfectionNotify.val(), 10) || 0, 100)
            );
        ...
        $textLevelNotify.on("change", function (e) {
            notifiedMinLevel = Math.max(
                0,
                Math.min(parseInt($textLevelNotify.val(), 10) || 0, 35)
            );
        ...
        $questsExcludePokemon.on("change", function (e) {
            ...
            questsExcludedPokemon = $questsExcludePokemon
                .val()
                .split(",")
                .map(Number)
                .sort(function (a, b) {
            ...
            reincludedQuestsPokemon = reincludedQuestsPokemon
                .concat(buffer)
                .map(String);
            updateMap();
        ...
        $questsExcludeEnergy.on("change", function (e) {
            ...
            questsExcludedEnergy = $questsExcludeEnergy
                .val()
                .split(",")
                .map(Number)
                .sort(function (a, b) {
            ...
            reincludedQuestsEnergy = reincludedQuestsEnergy
                .concat(buffer)
                .map(String);
            updateMap();
        ...
        $questsExcludeCandy.on("change", function (e) {
            ...
            questsExcludedCandy = $questsExcludeCandy
                .val()
                .split(",")
                .map(Number)
                .sort(function (a, b) {
            ...
            reincludedQuestsCandy = reincludedQuestsCandy
                .concat(buffer)
                .map(String);
            updateMap();
        ...
        $excludeRaidboss.on("change", function (e) {
            ...
            excludedRaidboss = $excludeRaidboss
                .val()
                .split(",")
                .map(Number)
                .sort(function (a, b) {
    ...
    $(".select-all").on("click", function (e) {
        ...
        parent
            .find(".search-number")
            .val(
                Array.from(Array(numberOfPokemon + 1).keys())
                    .slice(1)
                    .join(",")
            )
            .trigger("change");
    ...
    $(".select-all-item").on("click", function (e) {
        ...
        parent
            .find(".search-number")
            .val(
                Array.from(Array(numberOfItem + 1).keys())
                    .slice(1)
                    .join(",")
            )
            .trigger("change");
    ...
    $(".select-all-grunt").on("click", function (e) {
        ...
        parent
            .find(".search-number")
            .val(
                Array.from(Array(numberOfGrunt + 1).keys())
                    .slice(1)
                    .join(",")
            )
            .trigger("change");
    ...
    $(".select-all-egg").on("click", function (e) {
        ...
        parent
            .find(".search-number")
            .val(
                Array.from(Array(numberOfEgg + 1).keys())
                    .slice(1)
                    .join(",")
            )
            .trigger("change");
    ...
    function buildSwitchChangeListener(data, dataType, storageKey) {
        return function () {
            ...
            if (this.checked && storageKey !== "showQuestsWithTaskAR") {
                ...
                $.each(dataType, function (d, dType) {
                    $.each(data[dType], function (key, value) {
                        ...
                        if (storageKey === "showSpawnpoints") {
                            ...
                            storageKey !== "showRanges" &&
                            storageKey !== "showPlacementRanges"
                        ) {
                ...
                if (
                    storageKey === "showAllPokestops" ||
                    storageKey === "showLures" ||
                    storageKey === "showRocket" ||
                    storageKey === "showEventStops" ||
                    storageKey === "showQuests" ||
                    storageKey === "showQuestsWithTaskAR"
                ) {
                    lastpokestops = false;
                    updateMap();
                }
    ...
    $("#portals-switch").change(function () {
        ...
        return buildSwitchChangeListener(
            mapData,
            ["portals"],
            "showPortals"
        ).bind(this)();
    ...
    $("#pokemon-switch").change(function () {
        ...
        buildSwitchChangeListener(mapData, ["pokemons"], "showPokemon").bind(
            this
        )();
    });
    ...
    $("#ranges-switch").change(
        buildSwitchChangeListener(
            mapData,
            ["gyms", "pokemons", "pokestops"],
            "showRanges"
        )
    );
    $("#placement-ranges-switch").change(
        buildSwitchChangeListener(
            mapData,
            ["gyms", "pokestops"],
            "showPlacementRanges"
    ...
    $("#pokestops-switch").change(function () {
        ...
        buildSwitchChangeListener(mapData, ["pokestops"], "showPokestops").bind(
            this
        )();
    });
    ...
    $("#toast-delay-slider").on("input", function () {
        ...
        if (toastdelayslider === "0") {
            ...
            $("#toast-delay-set").text(
                i8ln("Popup autoclose") +
                    " " +
                    toastdelayslider / 1000 +
                    " " +
                    i8ln("seconds")
            );
            setTimeout(function () {
    ...
    $("#start-at-user-location-switch").change(function () {
        ...
        if (
            this.checked === true &&
            Store.get("startAtLastLocation") === true
        ) {
    ...
    $("#start-at-last-location-switch").change(function () {
        ...
        if (
            this.checked === true &&
            Store.get("startAtUserLocation") === true
        ) {
    ...
    pokemonTable = $("#pokemonTable").DataTable({
        ...
        language: {
            ...
            emptyTable:
                i8ln("Loading...") + ' <i class="fas fa-spinner fa-spin"></i>',
    ...
    switch (folder) {
        case "gym":
            if (iconpath["gymIndex"] === undefined) {
                ...
                const trainerCount =
                    typeof varArgs[0] === "undefined"
                        ? [""]
                        : varArgs[0] === 0
                        ? [""]
                        : ["_t" + varArgs[0], ""];
                const inBattle =
                    typeof varArgs[1] === "undefined"
                        ? [""]
                        : varArgs[1] === 0
                        ? [""]
                        : ["_b", ""];
                const isEx =
                    typeof varArgs[2] === "undefined"
                        ? [""]
                        : varArgs[2] === 0
                        ? [""]
                        : ["_ex", ""];
                search: for (const trainer of trainerCount) {
                        ...
                        for (const ex of isEx) {
                            ...
                            if (iconpath["gymIndex"].includes(requestedIcon)) {
                                if (!firstTry) {
                                    if (enableJSDebug) {
                                        console.log(
                                            "Repo has fallback gym icon! Returning: " +
                                                requestedIcon
                                        );
                                    }
                                }
        ...
        case "invasion":
            if (iconpath["invasionIndex"] === undefined) {
                if (enableJSDebug) {
                    console.log(
                        "No invasionIndex? Houston, we have a problem."
                    );
                }
            } else {
                ...
                if (iconpath["invasionIndex"].includes(requestedIcon)) {
                    ...
                    if (enableJSDebug) {
                        console.log(
                            "Repo is missing invasion icon: " + requestedIcon
                        );
                    }
                }
            }
        ...
        case "nest":
            if (iconpath["nestIndex"] === undefined) {
                ...
                if (iconpath["nestIndex"].includes(requestedIcon)) {
                    ...
                    if (enableJSDebug) {
                        console.log(
                            "Repo is missing nest icon: " + requestedIcon
                        );
                    }
                }
            }
            ...
            if (iconpath["pokemonIndex"] === undefined) {
                ...
                const costumeId =
                    typeof varArgs[2] === "undefined"
                        ...
                        : ["_c" + varArgs[2], ""];
                const genderId =
                    typeof varArgs[3] === "undefined"
                        ? [""]
                        : varArgs[3] === 0
                        ? [""]
                        : ["_g" + varArgs[3], ""];
                const shinyId =
                    typeof varArgs[4] === "undefined"
                        ? [""]
                        : varArgs[4] === 0
                        ? [""]
                        : ["_s", ""];
                const alignmentId =
                    typeof varArgs[5] === "undefined"
                        ? [""]
                        : varArgs[5] === 0
                        ? [""]
                        : ["_a" + varArgs[5], ""];
                search: for (const evolution of evolutionId) {
                            ...
                            for (const gender of genderId) {
                                for (const alignment of alignmentId) {
                                    for (const shiny of shinyId) {
                                        ...
                                        if (
                                            iconpath["pokemonIndex"].includes(
                                                requestedIcon
                                            )
                                        ) {
                                            if (!firstTry) {
                                                if (enableJSDebug) {
                                                    console.log(
                                                        "Repo has fallback pokemon icon! Returning: " +
                                                            requestedIcon
                                                    );
                                                }
                                            }
        ...
        case "pokestop":
            if (iconpath["pokestopIndex"] === undefined) {
                if (enableJSDebug) {
                    console.log(
                        "No pokestopIndex? Houston, we have a problem."
                    );
                }
            } else {
                ...
                const questId =
                    typeof varArgs[1] === "undefined"
                        ...
                        ? [""]
                        : ["_q", ""];
                search: for (const invasion of invasionId) {
                    for (const quest of questId) {
                        ...
                        if (iconpath["pokestopIndex"].includes(requestedIcon)) {
                            if (!firstTry) {
                                if (enableJSDebug) {
                                    console.log(
                                        "Repo has fallback pokestop icon! Returning: " +
                                            requestedIcon
                                    );
                                }
                            }
        ...
        case "raid/egg":
            if (iconpath["raidIndex"] === undefined) {
                ...
                if (enableJSDebug) {
                    console.log(
                        "No raidIndex->egg? Houston, we have a problem."
                    );
                }
            } else {
                ...
                const ex =
                    typeof varArgs[1] === "undefined"
                        ...
                        ? [""]
                        : ["_ex", ""];
                search: for (const hatched of isHatched) {
                    for (const e of ex) {
                        requestedIcon = `${eggLevel}${hatched}${e}${fileType}`;
                        if (
                            iconpath["raidIndex"]["egg"].includes(requestedIcon)
                        ) {
                            if (!firstTry) {
                                if (enableJSDebug) {
                                    console.log(
                                        "Repo has fallback raid->egg icon! Returning: " +
                                            requestedIcon
                                    );
                                }
                            }
        ...
        case "reward/item":
            if (iconpath["rewardIndex"] === undefined) {
                ...
                if (enableJSDebug) {
                    console.log(
                        "No rewardIndex->item? Houston, we have a problem."
                    );
                }
            } else {
                ...
                const itemAmount =
                    typeof varArgs[0] === "undefined"
                        ...
                        ? [""]
                        : ["_a" + varArgs[0], ""];
                search: for (const a of itemAmount) {
                    requestedIcon = `${itemId}${a}${fileType}`;
                    if (
                        iconpath["rewardIndex"]["item"].includes(requestedIcon)
                    ) {
                        if (!firstTry) {
                            if (enableJSDebug) {
                                console.log(
                                    "Repo has fallback reward->item icon! Returning: " +
                                        requestedIcon
                                );
                            }
                        }
        ...
        case "reward/mega_resource":
            if (iconpath["rewardIndex"] === undefined) {
                ...
                if (enableJSDebug) {
                    console.log(
                        "No rewardIndex->mega_resource? Houston, we have a problem."
                    );
                }
            } else {
                ...
                const megaAmount =
                    typeof varArgs[0] === "undefined"
                        ...
                        ? [""]
                        : ["_a" + varArgs[0], ""];
                search: for (const a of megaAmount) {
                    requestedIcon = `${megaPokemon}${a}${fileType}`;

                    if (
                        iconpath["rewardIndex"]["mega_resource"].includes(
                            requestedIcon
                        )
                    ) {
                        if (!firstTry) {
                            if (enableJSDebug) {
                                console.log(
                                    "Repo has fallback reward->mega_resource icon: " +
                                        requestedIcon
                                );
                            }
                        }
        ...
        case "reward/stardust":
            if (iconpath["rewardIndex"] === undefined) {
                ...
                if (enableJSDebug) {
                    console.log(
                        "No rewardIndex->stardust? Houston, we have a problem."
                    );
                }
            } else {
                ...
                if (
                    iconpath["rewardIndex"]["stardust"].includes(requestedIcon)
                ) {
                    icon = requestedIcon;
                } else {
                    if (enableJSDebug) {
                        console.log(
                            "Repo is missing reward->stardust icon: " +
                                requestedIcon
                        );
                    }
                }
        ...
        case "reward/experience":
            if (iconpath["rewardIndex"] === undefined) {
                ...
                if (enableJSDebug) {
                    console.log(
                        "No rewardIndex->experience? Houston, we have a problem."
                    );
                }
            } else {
                ...
                if (
                    iconpath["rewardIndex"]["experience"].includes(
                        requestedIcon
                    )
                ) {
                    icon = requestedIcon;
                    ...
                    if (enableJSDebug) {
                        console.log(
                            "Repo is missing reward->experience icon: " +
                                requestedIcon
                        );
                    }
                }
        ...
        case "reward/candy":
            if (iconpath["rewardIndex"] === undefined) {
                ...
                if (enableJSDebug) {
                    console.log(
                        "No rewardIndex->candy? Houston, we have a problem."
                    );
                }
            } else {
                ...
                const candyAmount =
                    typeof varArgs[0] === "undefined"
                        ...
                        ? [""]
                        : ["_a" + varArgs[0], ""];
                search: for (const a of candyAmount) {
                    requestedIcon = `${pokemonIdCandy}${a}${fileType}`;
                    if (
                        iconpath["rewardIndex"]["candy"].includes(requestedIcon)
                    ) {
                        if (!firstTry) {
                            if (enableJSDebug) {
                                console.log(
                                    "Repo has fallback reward->candy icon! Returning: " +
                                        requestedIcon
                                );
                            }
                        }
        ...
        case "team":
            if (iconpath["teamIndex"] === undefined) {
                ...
                if (iconpath["teamIndex"].includes(requestedIcon)) {
                    ...
                    if (enableJSDebug) {
                        console.log(
                            "Repo is missing team icon: " + requestedIcon
                        );
                    }
                }
            }
        ...
        case "type":
            if (iconpath["typeIndex"] === undefined) {
                ...
                if (iconpath["typeIndex"].includes(requestedIcon)) {
                    ...
                    if (enableJSDebug) {
                        console.log(
                            "Repo is missing type icon: " + requestedIcon
                        );
                    }
                }
            }
...
function download(filename, text) {
    ...
    element.setAttribute(
        "download",
        filename + "_" + moment().format("DD-MM-YYYY HH:mm")
    );
...
function loadUser(engine) {
    return $.ajax({
        ...
        error: function error(xhr, status, errorThrown) {
            ...
            sendToast(
                "danger",
                i8ln("Failed to refresh session"),
                i8ln("Manually reload the page."),
                "true"
            );
        },
        complete: function complete() {
